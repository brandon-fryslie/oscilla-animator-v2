...
# 2. Reset Statuses
# If they are CLOSED, they don't show up in ready.
# If they are IN_PROGRESS, they might be skipped or confused.
# Let's keep closed ones closed, but ensure id1..id6 are properly marked.
# Wait, actually id1..id6 ARE finished now.
# So p0_1 should be the first open one.

# 3. Add dependencies: dep add <dependent> <blocker>
# Syntax: lnks dep add <dependent> <blocker> means <blocker> blocks <dependent>.
