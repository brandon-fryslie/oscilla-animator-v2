...
**Goal:** Define socket types that map to Oscilla's SignalType system and enforce connection compatibility.
...
// Map from SignalType to Socket
export function getSocketForSignalType(signalType: SignalType): OscillaSocket {
  const key = `${signalType.cardinality}_${signalType.payloadType}`;
...
export class OscillaNode extends ClassicPreset.Node {
  ...
  constructor(blockDef: BlockDef, blockId: BlockId, displayName?: string | null) {
    ...
    for (const input of blockDef.inputs) {
      const socket = getSocketForSignalType(input.signalType);
    ...
    for (const output of blockDef.outputs) {
      const socket = getSocketForSignalType(output.signalType);
