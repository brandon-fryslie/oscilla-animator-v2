...
- Maps directly to our SignalType system
...
### 3.1 Socket Types → SignalType Mapping

Our SignalType system has axes: PayloadType, Cardinality, Temporality, Binding, Extent
  ...
  constructor(
    ...
    for (const input of inputs) {
      this.addInput(input.id, new ClassicPreset.Input(
        getSocketForType(input.signalType),
        input.label,
    ...
    for (const output of outputs) {
      this.addOutput(output.id, new ClassicPreset.Output(
        getSocketForType(output.signalType),
        output.label
      ));
    }
  }
...
2. **P1:** Socket type definitions mapped to SignalType
