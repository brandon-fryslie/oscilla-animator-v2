...
interface InputDef {
  ...
  readonly type?: CanonicalType;       // Type for wirable ports
...
interface OutputDef {
  readonly id: string;
  readonly label?: string;          // Display label (defaults to id)
  readonly type: CanonicalType;        // Required for outputs
...
registerBlock({
  type: 'Add',
  inputs: {
    a: { label: 'A', type: canonicalType('float'), exposedAsPort: true },
    b: { label: 'B', type: canonicalType('float'), exposedAsPort: true },
  },
  outputs: {
