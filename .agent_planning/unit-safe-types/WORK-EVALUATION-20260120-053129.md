...
1. All TimeRoot phase outputs use `canonicalType('phase')`
2. Oscillator phase input uses `canonicalType('phase')`
3. All field blocks with phase inputs use `canonicalType('phase')`
...
9. No `canonicalType('float')` used for values that are semantically phase [0,1)
...
{ id: 'phaseA', label: 'Phase A', type: canonicalType('phase') },
{ id: 'phaseB', label: 'Phase B', type: canonicalType('phase') },

const phaseA = ctx.b.sigTime('phaseA', canonicalType('phase'));
const phaseB = ctx.b.sigTime('phaseB', canonicalType('phase'));
```

**Evidence:** Both phaseA and phaseB outputs now use `canonicalType('phase')` instead of `canonicalType('float')`. Includes clear documentation comment explaining the [0,1) range.
...
{ id: 'phase', label: 'Phase', type: canonicalType('phase') },
```

**Evidence:** Oscillator phase input now uses `canonicalType('phase')`. Documentation explains the kernel conversion to radians.
...
{ id: 'phase', label: 'Phase', type: canonicalType('phase'), defaultSource: defaultSourceTimeRoot('phaseA') },
```
...
**Evidence:** All three field blocks now use `canonicalType('phase')` for their phase inputs, with documentation and appropriate default sources.
...
### ✅ Working
- **TimeRoot phase outputs**: Both phaseA and phaseB use `canonicalType('phase')` with clear documentation
- **Oscillator phase input**: Uses `canonicalType('phase')` with kernel conversion explanation
- **Field blocks phase inputs**: All 3 field blocks (FieldPulse, FieldAngularOffset, FieldHueFromPhase) use `canonicalType('phase')`
...
- **Consistency**: No `canonicalType('float')` misuse for phase values
