...
outputs: [
  ...
  { id: 'phaseA', label: 'Phase A', type: signalType('float') },  // Should be 'phase'!
  { id: 'phaseB', label: 'Phase B', type: signalType('float') },  // Should be 'phase'!
],
```
...
```typescript
inputs: [
  { id: 'phase', label: 'Phase', type: signalType('float') },  // Should be 'phase'!
],
```
...
2. **Add Unit Annotation** - Extend SignalType with optional `unit` field
...
|------|--------|------------|
| Breaking changes to SignalType | Medium | Additive change (optional `unit` field) |
