...
### Concrete Example (MakeColorOKLCH)

```
Expr[41] kernel/zip card=many   ← Add block output (correct: field)
Expr[42] kernel/map card=one    ← MakeColorOKLCH.wrap01(hInput) with hardcoded floatType
                                   ^^^ BUG: card=one but input is card=many
```

`make-color-oklch.ts:47`: `const floatType = canonicalType(FLOAT, unitTurns(), undefined, contractWrap01());`
...
- **MakeColorOKLCH** (`color/make-color-oklch.ts`) — creates `floatType` with `canonicalType(FLOAT, ...)`, uses for wrap/clamp intermediates
