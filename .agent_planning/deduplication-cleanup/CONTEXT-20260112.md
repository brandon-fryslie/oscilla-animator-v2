...
- `spec.portType: SignalType` - the target port's expected type
...
**Validation needed:**
1. Look up each writer's SignalType from the source block's output slot
...
1. Rename `createStubProgramIR` → `createProgramIR` (remove "stub")
