...
**Acceptance Criteria:**
- [ ] TimeRoot block has `pulse` output: `{ label: 'Pulse', type: signalType('float') }`
...
**Technical Notes:**
- Spec says pulse is `unit` payload with `discrete` temporality. However, our signal system is scalar. For v1, pulse is a float (0.0 or 1.0) with continuous temporality. A proper `unit`/`discrete` model would require event system infrastructure (Phase 3). Use `signalType('float')` for now — this is a pragmatic decision, not a spec violation per se (the value semantics are preserved).
...
- [ ] `allocTypedSlot(signalType('color'))` reserves 4 consecutive f64 entries (slotCounter += 4)
- [ ] `slotMeta` generation emits stride for each entry
- [ ] `pass3-time` palette allocation uses `allocTypedSlot(signalType('color'))` instead of bare `allocSlot()`
...
- **Stride derivation**: stride is determined by `PayloadType` in `SignalType`. The function `payloadStride()` is the canonical mapping:
  ```
