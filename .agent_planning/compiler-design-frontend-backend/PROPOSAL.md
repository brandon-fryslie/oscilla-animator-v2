...
•	Frontend may insert adapter blocks automatically during normalization.
•	Inserted adapters are real blocks (from a constrained “adapter subset”), visible in the UI, and become part of the normalized graph.
•	Backend consumes a fully normalized, adapter-expanded, fully typed graph and must not contain any adapter-specific logic.
...
1. Terms
   ...
   •	all required conversions already exist as adapter blocks
