...
- But we exposed a **pre-existing issue**: `pass2-adapters.ts` is NOT inserting `Broadcast` adapters when a Signal (one) connects to a Field (many) input
...
- Fix the adapter insertion issue (Broadcast not being created)
...
### 3. Adapter Pass Not Inserting Broadcast (BLOCKING)

`pass2-adapters.ts` in graph passes should automatically insert `Broadcast` when connecting Signal→Field, but it's not doing so. This is why steel-thread and other tests fail with cardinality mismatch.
...
2. Investigate why `pass2-adapters.ts` is not inserting Broadcast
