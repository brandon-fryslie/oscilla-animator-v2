...
V1 (oscilla-animator_codex) established strong architectural foundations that V2 successfully preserves. The analysis identifies **7 patterns worth preserving** and **6 anti-patterns to avoid**. The god block refactor (PositionSwirl → 5 composable blocks) demonstrates V2's commitment to simplicity and composability.
...
### 1. Single Linear Compiler Pipeline ✅ PRESERVED
...
**V2 Status**: ✅ **PRESERVED** in `src/compiler/compile.ts`
...
- ✅ ONE SOURCE OF TRUTH: Single compile entry point
- ✅ SINGLE ENFORCER: Type checking in one place (TypeChecker.ts)
- ✅ ONE-WAY DEPENDENCIES: Graph → Compiler → IR → Runtime → Render

**Recommendation**: **Maintain this pattern**. Do not introduce multi-pass systems unless absolutely necessary.

---

### 2. Block Registry Pattern ✅ PRESERVED
...
**V2 Status**: ✅ **PRESERVED** in `src/compiler/blocks/registry.ts`
...
interface IRBuilder {
  ...
  Broadcast(signal, type): FieldExprId
...
**V2 Status**: ✅ **PRESERVED** in `src/compiler/ir/IRBuilder.ts` and `IRBuilderImpl.ts`
...
### 4. Graph Normalization Step ✅ PRESERVED (BASIC)

**V1 Pattern**:
```
Patch (user graph) → Normalize → NormalizedPatch (dense indices)
```

**Normalization Responsibilities**:
- Convert Map-based blocks to dense array [0..N]
- Convert edge references from blockId → array index
- Validate graph structure (no dangling edges)
- (Future: Expand composites, materialize default sources, expand lenses/adapters)

**V2 Status**: ✅ **PRESERVED** in `src/graph/normalize.ts`
...
### 5. Composable Primitive Blocks ✅ ACHIEVED in V2
...
5. **FieldPolarToCartesian** (55 lines, 2 inputs) - Polar→Cartesian conversion

**HueRainbow → 2 blocks**:
1. **FieldHueFromPhase** (50 lines, 2 inputs) - Calculates hue = phase + id01
2. **HsvToRgb** (40 lines, 3 inputs) - HSV→RGB kernel conversion

**Why It Works**:
- Each block has ≤5 inputs (complexity limit enforced)
...
**V2 Status**: ✅ **ACHIEVED** (completed in architectural refactor, 2026-01-05)
...
### 6. IR-as-Data (Not Closures) ✅ SUPERIOR to V1
...
**V2 Status**: ✅ **SUPERIOR** - V2's IR-as-data is architecturally better than any closure-based approach
...
### 7. Single TypeChecker Enforcer ✅ PRESERVED
...
**Why It Works**:
- ✅ SINGLE ENFORCER architectural law
...
**V2 Status**: ✅ **PRESERVED** in `src/compiler/passes/TypeChecker.ts`
...
// Inside: Does golden angle + spin + radius + polar→cartesian
```
...
**V2 Solution**: ✅ **ELIMINATED** - Split into 5 composable blocks (see Pattern #5)
...
### 2. Duplicate Type Systems ❌ TO BE REMOVED
...
|-------------------------------------|--------|------------------------|
| `src/types/index.ts` (255 lines)    | ✅ ACTIVE | Main compiler pipeline |
| `src/core/types.ts` (249 lines)     | ❌ UNUSED | Never imported         |
| `src/blocks/registry.ts` (94 lines) | ❌ UNUSED | Never imported         |
...
**V2 Status**: ❌ **STILL EXISTS** (TODO: Delete unused files)
...
### 3. Incomplete Passes Framework ❌ TO BE REMOVED OR COMPLETED
...
**V2 Status**: ❌ **STILL EXISTS** (TODO: Delete or integrate)
...
### 4. Unused Transform Registry ❌ TO BE REMOVED OR IMPLEMENTED
...
**V2 Status**: ❌ **STILL EXISTS** (TODO: Delete or implement)
...
### 5. Ambiguous File Organization ⚠️ NEEDS IMPROVEMENT
...
```
src/compiler/blocks/
├── time/           # InfiniteTimeRoot
├── signal/         # ConstFloat, AddSignal, MulSignal, Oscillator, MousePosition
├── domain/         # DomainN, GridDomain, Broadcast, FieldMap, FieldZipSig
├── render/         # 12+ render blocks
...
```
src/compiler/blocks/
├── time/           # TimeRoot variants
├── signal/         # Signal-world blocks
├── field/          # Field-world blocks
├── event/          # Event-world blocks (when implemented)
├── stateful/       # UnitDelay, Lag, Phasor, SampleAndHold (when implemented)
├── transforms/     # Lens/adapter blocks (when implemented)
├── composites/     # Composite blocks (when implemented)
└── registry.ts
```

---

### 6. Spec vs Implementation Drift ⚠️ SIGNIFICANT GAP
...
| --- | --- | --- | --- |
| **Core Type System** | compiler/01-type-system.md | ✅ Partial (src/types/index.ts) | Missing domain conversions |
| **Time Model** | graph/02-time.md | ✅ Partial (TimeModel exists) | No TimeRoot integration |
| **Buses & Rails** | graph/03-buses.md | ❌ None | Needs graph normalization |
| **Compilation** | compiler/04-compilation.md | ✅ Partial (compile.ts) | No full normalization |
| **Runtime** | runtime/05-runtime.md | ✅ Partial (ScheduleExecutor) | No hot swap |
| **Blocks** | graph/06-blocks.md | ✅ Partial (40 blocks) | No composites |
| **Transforms** | graph/07-transforms.md | ❌ None (0 of 50+) | Needs lens system |
| **Primitives** | graph/08-primitives-composites.md | 🚧 Stub | Not enumerated |
| **Renderer** | renderer/09-renderer.md | ✅ Minimal (Canvas2D) | Only squares |
| **Stateful Blocks** | graph/stateful-blocks.md | ❌ None (0 of 4) | Ambiguity on which 4 |
| **Default Sources** | (invariants) | ❌ None | Needs normalization |
| **Graph Normalization** | 16-Graph-Normalization.md | ❌ None | Core blocker |
| **Phase Matching** | time/10-phase-matching-system.md | ❌ None | Advanced feature |
...
| --- | --- | --- | --- |
| **Patch** | Patch | ✅ Unchanged | User-facing graph representation |
| **BlockDef** | BlockDef | ✅ Unchanged | Block definition with lower() function |
| **IRProgram** | IRProgram | ✅ Unchanged | Core IR structure preserved |
| **TimeModel** | TimeModel | ✅ Partial | Exists but needs TimeRoot integration per spec |
| **Signal world** | Signal world | ✅ Preserved | Signal<T> type unchanged |
| **Field world** | Field world | ✅ Preserved | Field<T> type unchanged |
| **Scalar world** | Scalar world | ⚠️ Spec-only | Not yet in implementation |
| **Event world** | Event world | ⚠️ Spec-only | Not yet in implementation |
| **Connection** | Edge | ✅ Simplified | V1 had complex Connection types, V2 unified to Edge |
| **Publisher/Listener** | Edge (multi-writer) | ⚠️ Partial | V1 had bus complexity, V2 spec simplifies but not implemented |
| **Lenses/Adapters** | Transform blocks | ❌ Spec-only | V1 explored, V2 spec defines, not implemented |
| **Default Sources** | Default Source blocks | ❌ Spec-only | V1 didn't have, V2 spec requires, not implemented |
| **Composites** | Composite blocks | ❌ Spec-only | Not in V1 or V2 implementation |
| **Stateful blocks** | 4 canonical stateful primitives | ❌ Spec-only | V1 had some, V2 spec locks to 4, not implemented |
| **Type checking** | TypeChecker.ts | ✅ Improved | Single enforcer pattern better than V1 |
| **Dependency order** | Kahn's algorithm | ✅ Unchanged | Same algorithm, cleaner implementation |
| **Block lowering** | BlockLower function | ✅ Unchanged | lower() pattern preserved |
| **IR Builder** | IRBuilder interface | ✅ Improved | V2 has cleaner abstraction |
| **Runtime** | ScheduleExecutor | ✅ Improved | V2 has better separation |
| **Renderer** | Canvas2DRenderer | ✅ Minimal | V2 has basic renderer, spec requires more |
| **Graph normalization** | NormalizedPatch | ✅ Basic | V2 has basic, spec requires expansion |
| **Rails** | Bus blocks (system) | ❌ Spec-only | New in V2 spec, not implemented |
| **Buses** | Bus blocks (user) | ❌ Spec-only | V1 had complex system, V2 spec simplifies, not implemented |
| **Domains** | Domain types | ✅ Partial | V2 has DomainN/GridDomain, spec requires more |
...
1. ✅ **PRESERVE**: Single linear compiler pipeline
2. ✅ **PRESERVE**: Block registry pattern
3. ✅ **PRESERVE**: IRBuilder pattern
4. ✅ **PRESERVE**: Graph normalization (expand per spec)
5. ✅ **PRESERVE**: Composable primitive blocks
6. ✅ **PRESERVE**: IR-as-data (never closures)
7. ✅ **PRESERVE**: Single TypeChecker enforcer

8. ❌ **DELETE**: `src/core/types.ts` (duplicate type system)
9. ❌ **DELETE**: `src/blocks/registry.ts` (unused duplicate)
10. ❌ **DECIDE**: `src/compiler/passes-v2/` (delete or integrate - recommend delete)
11. ❌ **DECIDE**: `src/transforms/` (delete or implement - defer to Phase 2)

12. ⚠️ **IMPROVE**: Folder structure (align with spec worlds)
13. ⚠️ **AUDIT**: All code against spec (identify conformance gaps)
