...
- Only two TimeRoot kinds: finite and infinite
...
- Linear, predictable
- No hidden passes
- Clear separation: graph → compiler → IR → runtime → render
...
│   ├── blocks/registry.ts (✅ 40 blocks registered)
│   ├── ir/ (✅ IR types and builder)
│   └── passes/ (TypeChecker only)
├── runtime/
│   ├── ScheduleExecutor.ts (✅ frame execution)
│   ├── SignalEvaluator.ts (✅ signal computation)
│   ├── Materializer.ts (✅ field evaluation)
│   └── BufferPool.ts (✅ memory management)
├── render/
│   └── Canvas2DRenderer.ts (✅ 2D squares only)
├── graph/
│   ├── Patch.ts (✅ user graph)
│   └── normalize.ts (✅ basic normalization)
└── types/
    └── index.ts (✅ active type system)
```

**40 Blocks Implemented** (from registry):
- Time (2): InfiniteTimeRoot, FiniteTimeRoot
- Signals (5): ConstFloat, AddSignal, MulSignal, Oscillator, MousePosition
- Domain (5): DomainN, GridDomain, FieldBroadcast, FieldMap, FieldZipSig
