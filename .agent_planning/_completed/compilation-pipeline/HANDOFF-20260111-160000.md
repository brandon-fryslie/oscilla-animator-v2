...
registerBlock({
  ...
  inputs: [
    { id: 'inputPort', label: 'Input', type: signalType('float') },
  ],
  outputs: [
    { id: 'outputPort', label: 'Output', type: signalType('float') },
  ],
  params: {},
});
...
registerBlockType({
  type: 'BlockName',
  inputs: [{ portId: 'inputPort', type: signalType('float') }],
  outputs: [{ portId: 'outputPort', type: signalType('float') }],
  lower: ({ ctx, inputsById, config }) => {
    // Use ctx.b methods: sigMap, sigZip, fieldSource, fieldBroadcast, etc.
...
- Use `signalType('float')` for scalar signals
