...
| 2 | Type Assignment | Assign CanonicalType with AxisTag per port |
...
- `bridgeExtentToAxesDescIR()` - CanonicalType.extent → AxesDescIR
- `payloadTypeToShapeDescIR()` - PayloadType → ShapeDescIR

**Acceptance Criteria:**
- [ ] `offset` is required (no `?`)
- [ ] All 5 axes defined
- [ ] Bridge functions handle all CanonicalType variants
...
- Assign `CanonicalType = { payload, extent }` per port
...
- [ ] Pass 2 assigns CanonicalType to all ports
...
- P0-P3 complete (CanonicalType with 5-axis) ✓
