...
**Acceptance Criteria**:
- [ ] Output names match spec (tMs, phaseA, phaseB, pulse, palette, energy)
- [ ] CanonicalType for tMs uses `int` payload
...
### Type System State
- Codebase has TWO parallel type systems: `TypeDesc` (old) and `CanonicalType` (new/canonical)
- TimeRoot blocks already use `CanonicalType` via `sigType()` helper
- Bridge functions exist in `src/compiler/ir/bridge.ts`
- **Action**: Use `CanonicalType` for all new code
...
**Risk**: Old TypeDesc vs new CanonicalType creates friction
**Mitigation**: Use CanonicalType exclusively in new code, bridge.ts handles conversion
