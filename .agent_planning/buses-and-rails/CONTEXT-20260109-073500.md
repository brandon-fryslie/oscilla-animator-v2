...
| `src/core/canonical-types.ts` | CanonicalType 5-axis system | Reference for type patterns |
...
```typescript
export function eventType(payload: PayloadType = 'float'): CanonicalType {
...
function injectRails(blocks: Map<BlockId, Block>, edges: Edge[]): { blocks: Map<BlockId, Block>, edges: Edge[] } {
  ...
  const railDefs: Array<{ id: RailId, type: string, inputType: CanonicalType, sourcePort: string }> = [
