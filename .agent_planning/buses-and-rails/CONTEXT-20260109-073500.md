...
| `src/compiler/blocks/time/FiniteTimeRoot.ts` | Finite time source | **MODIFY outputs** |
...
### Target Output Structure (FiniteTimeRoot)

Same as InfiniteTimeRoot PLUS:
```typescript
{ portId: portId('progress'), type: sigType('unit') },  // 0-1 over duration
```
...
function injectRails(blocks: Map<BlockId, Block>, edges: Edge[]): { blocks: Map<BlockId, Block>, edges: Edge[] } {
  ...
  const timeRoot = [...blocks.values()].find(b =>
    b.type === 'InfiniteTimeRoot' || b.type === 'FiniteTimeRoot'
  );
...
3. **`src/compiler/blocks/time/FiniteTimeRoot.ts`** - Same fixes as InfiniteTimeRoot (P1)
