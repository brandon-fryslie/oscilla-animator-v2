...
- `src/compiler/blocks/time/FiniteTimeRoot.ts` - Outputs: t, dt, progress, phase, pulse, energy
- Both use IR builder to emit time expressions

**Specification Gaps**:

1. **Output Names Wrong**:
   - Spec says: `tMs` (milliseconds), `phaseA`, `phaseB`, `progress` (for finite only), `pulse`, `palette`
...
5. Bus-to-block edge rewriting (bus publish/subscribe → wires)
...
**Issue**:
- TimeRoot/FiniteTimeRoot blocks registered with SignalType (canonical)
...
3. RailBlock definitions (5 blocks)
   ├→ Palette color cycle generation
   └→ TimeRoot fixes (correct output names, types)
   ↓
4. GraphNormalization enhancements
   ├→ Auto-inject TimeRoot
   ├→ Auto-inject 5 rails
   ├→ Auto-inject DefaultSource blocks
   └→ Assign roles to all blocks/edges
   ↓
5. BusBlock definition
   ├→ BusId type and indexing
   └→ CombineMode for inputs
   ↓
...
4. **Discrete Pulse**: Spec says `pulse: one + discrete + unit` but:
   ...
   - What about finite TimeRoot pulse vs infinite?
...
|-----------|--------|-----------|-----------------|
| BlockRole | ❌ MISSING | 02-block-system | None |
| DerivedBlockMeta | ❌ MISSING | 02-block-system | None |
| TimeRoot blocks | 🟡 PARTIAL | 03-time-system | InfiniteTimeRoot, FiniteTimeRoot (wrong output names/types) |
| Rails (5 blocks) | ❌ MISSING | 03-time-system | None |
| Palette rail | ❌ MISSING | 03-time-system | None |
| BusBlock | ❌ MISSING | 03-buses | None |
| MultiWriter inputs | 🟡 PARTIAL | 03-buses | resolveWriters.ts but no integration |
| CombineMode | 🟡 PARTIAL | 03-buses | Incomplete type, no custom support |
| GraphNormalization | 🟡 MINIMAL | 16-Graph-Normalization | Basic structure only |
| EdgeRole | ❌ MISSING | 02-block-system | None |
| Default sources | ❌ MISSING | 02-block-system | Mentioned but not implemented |
| Type system | 🟡 DUAL | 01-type-system | SignalType + TypeDesc (not integrated) |
