...
```
┌──────────────────────────────────────────────────────────────┐
│ [Search blocks...]                    [Show: All ▼]          │
├──────────────────────────────────────────────────────────────┤
│ BLOCKS                                                       │
│ ┌────────────────┬────────┬────────┬────────┬─────────────┐  │
│ │ Block          │ Type   │ Domain │ Inputs │ Outputs     │  │
│ ├────────────────┼────────┼────────┼────────┼─────────────┤  │
│ │ ▸ time         │ Infin..│   -    │   0    │ phaseA (2)  │  │
│ │ ▸ domain       │ Domain │ partic │   0    │ domain (8)  │  │
│ │ ▸ id01         │ Field..│ partic │   1    │ id01 (5)    │  │
│ │ ▸ radiusPulse  │ Field..│ partic │   4    │ value (1)   │  │
│ │ ...            │        │        │        │             │  │
│ └────────────────┴────────┴────────┴────────┴─────────────┘  │
├──────────────────────────────────────────────────────────────┤
│ RAILS                                                        │
│ ┌────────────────┬────────┬──────────┬───────────┐           │
│ │ Rail           │ Type   │ Default  │ Combine   │           │
│ ├────────────────┼────────┼──────────┼───────────┤           │
│ │ phaseA         │ Signal │ time.out │ last      │           │
│ │ energy         │ Signal │ 0.5      │ sum       │           │
│ └────────────────┴────────┴──────────┴───────────┘           │
└──────────────────────────────────────────────────────────────┘
```
...
- **Click expand (▸)**: Show block's ports and connections inline
...
```
│ ▾ radiusPulse    │ FieldPulse │ particles │        │             │
│   ├─ phase     ← │ Signal     │           │ time.phaseA          │
│   ├─ id01      ← │ Field<num> │           │ id01.id01            │
│   ├─ base      ← │ Signal     │           │ radiusBase.out       │
│   ├─ amplitude ← │ Signal     │           │ radiusAmplitude.out  │
│   ├─ spread    ← │ Signal     │           │ radiusSpread.out     │
│   └─ value     → │ Field<num> │           │ → effectiveRadius    │
```
...
│ INPUTS                                  │
│ ┌─────────────────────────────────────┐ │
│ │ phase    Signal<number>         ● ──┼─│→ time.phaseA
│ │ id01     Field<number>          ● ──┼─│→ id01.id01
│ │ base     Signal<number>         ● ──┼─│→ radiusBase.out
│ │ amplitude Signal<number>        ● ──┼─│→ radiusAmplitude.out
│ │ spread   Signal<number>         ● ──┼─│→ radiusSpread.out
│ └─────────────────────────────────────┘ │
├─────────────────────────────────────────┤
│ OUTPUTS                                 │
│ ┌─────────────────────────────────────┐ │
│ │ value    Field<number>          ● ──┼─│→ effectiveRadius.radius
│ └─────────────────────────────────────┘ │
├─────────────────────────────────────────┤
│ DEFAULT SOURCES                         │
│ ┌─────────────────────────────────────┐ │
...
- **Click port indicator (●/○)**: (Future) Start wiring mode
...
6. [ ] Row selection → updates global selection state
...
├── AppLayout.ts          # Existing - panel infrastructure
├── TabbedContent.ts      # Existing - tabs
├── components/
│   ├── TableView.ts      # Main table component
│   ├── BlockInspector.ts # Inspector for blocks
│   ├── BlockLibrary.ts   # Library panel
│   ├── DomainsPanel.ts   # Domains editor
│   └── shared/
│       ├── PortList.ts   # Reusable port listing
│       └── TypeBadge.ts  # Type indicator component
├── state/
│   └── selection.ts      # Selection state management
└── registry/
    └── blockTypes.ts     # Block type definitions for library
```
