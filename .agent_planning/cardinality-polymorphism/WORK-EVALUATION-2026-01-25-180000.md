...
All 7 blocks use `canonicalType()` for main I/O ports:

| Block | Main Ports | Type | Status |
|-------|-----------|------|--------|
| Pulse | id01, value | `canonicalType('float')` | PASS |
| RadiusSqrt | id01, radius, out | `canonicalType('float')` | PASS |
| Jitter2D | pos, rand, out | `canonicalType('vec2'/'float')` | PASS |
| HueFromPhase | id01, hue | `canonicalType('float')` | PASS |
| SetZ | pos, z, out | `canonicalType('vec3'/'float')` | PASS |
| FieldPolarToCartesian | angle, radius, pos | `canonicalType('float'/'vec3')` | PASS |
| FieldCartesianToPolar | pos, angle, radius | `canonicalType('vec3'/'float')` | PASS |
...
- P1: All 7 PRESERVE blocks use canonicalType() for main I/O ports
