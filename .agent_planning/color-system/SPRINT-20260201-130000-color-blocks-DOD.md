...
### ColorPicker
- Outputs constant color with unit oklch
- h=0 s=1 l=0.5 a=1 → color value [0, 1, 0.5, 1]
- h=1.5 → wraps to 0.5, s=-0.1 → clamps to 0

### MakeColorOKLCH
- Input (0.25, 0.8, 0.6, 1.0) → color+oklch [0.25, 0.8, 0.6, 1.0]
- h wrapping: input h=1.3 → stored as 0.3
- s/l/a clamping: input s=1.5 → stored as 1.0

### SplitColorOKLCH
- Round-trip: MakeColorOKLCH(h,s,l,a) → SplitColorOKLCH → (h,s,l,a) unchanged
...
- Shortest-arc: mix(oklch(0.1,...), oklch(0.9,...), 0.5) → oklch(0.0,...) not oklch(0.5,...)
...
- Adapter auto-insertion works when wiring oklch output to rgba01 input

### Integration
- Can wire: ColorPicker → HueShift → MixColor → OklchToRgba → (render sink)
