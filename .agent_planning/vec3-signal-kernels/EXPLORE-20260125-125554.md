...
const result = ctx.b.sigZip([x, y, z], vec3Fn, canonicalType('vec3', unitWorld3()));
```
...
const result = ctx.b.sigZip([pos.id, rand.id, amountXSig, amountYSig, amountZSig], jitterFn, canonicalType('vec3', unitWorld3()));
```
...
const result = ctx.b.sigZip([pos.id, z.id], setZFn, canonicalType('vec3'));
```
