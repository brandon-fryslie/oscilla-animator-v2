...
1. Allocate `slot = allocSlot(S)`
2. Compute scalar component expressions `c0..c(S-1)` as SigExprIds
3. Emit `stepSlotWriteStrided(slot, [c0..cS-1])`
4. Return the output value as:
...
1. The input value will have `{ slot, stride }`
2. Read components by slot indexing using existing slot SigExpr:
...
2. strided slot write step
3. expose output as slot+stride
