...
- Type resolution with CanonicalType (payload + 5-axis Extent)
...
6. **MappingState type missing** - Spec 11-continuity-system.md §3.3 defines mapping types (identity, byId, byPosition) used by StepContinuityMapBuild
...
4. **StepContinuityMapBuild.outputMapping** - Stored as string key, but spec §5.1 shows output mapping should be MappingState type
