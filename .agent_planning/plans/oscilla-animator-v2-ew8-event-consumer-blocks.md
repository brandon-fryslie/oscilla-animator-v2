...
export interface SigExprEventRead {
  ...
  readonly type: SignalType;  // Always signalType('float')
}
```
...
registerBlock({
  ...
  inputs: {
    event: { label: 'Event', type: signalType('bool') },  // Event input port
  },
  outputs: {
...
registerBlock({
  ...
  inputs: {
    ...
    trigger: { label: 'Trigger', type: signalType('bool') },  // Event input port
