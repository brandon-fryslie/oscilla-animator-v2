...
| signalType factory | PASS | Available in canonical-types |
...
**Evidence**: `signalType()` factory at `/Users/bmf/code/oscilla-animator-v2/src/core/canonical-types.ts`
**Issues**: Need to verify signalType can produce all needed configurations

**Fix approach**:
```typescript
// Instead of: { payload: 'vec2', cardinality: 'many' } as const ... type as any
import { signalTypeField } from '../../core/canonical-types';
const type = signalTypeField('vec2');  // Creates proper SignalType for fields
```
...
3. **Category 4: Field Kernel Type Params** (THIRD)
   - No dependencies
   - Uses existing signalType factory
