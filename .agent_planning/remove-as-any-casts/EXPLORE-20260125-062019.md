...
**Evidence**: The `as const` is correct for literal types, but the resulting object may not match the expected `SignalType` shape.

**Fix approach**: Use `signalType()` factory from canonical-types:
```typescript
import { signalType } from '../../core/canonical-types';
const type = signalType('vec2');  // or with cardinality
```
...
4. **Medium risk**: Field kernel params - may need SignalType construction
...
- Category 4 (field kernel) depends on signalType factory coverage
