...
export interface FieldExprBroadcast {
  ...
  readonly type: CanonicalType;      // Output: field type
}
```
...
export interface SigExprReduceField {
  ...
  readonly type: CanonicalType;      // Output: signal type
}
```
...
export interface SigExprReduceField {
  ...
  readonly type: CanonicalType;      // Output: signal type (cardinality=one)
}
```
...
```typescript
ReduceField(field: FieldExprId, op: ReduceOp, type: CanonicalType): SigExprId {
...
registerBlock({
  ...
  inputs: {
    signal: { label: 'Signal', type: canonicalType(...) },
  },
  outputs: {
...
registerBlock({
  ...
  outputs: {
    signal: { label: 'Scalar', type: canonicalType(payloadVar(...)) },
  },
