...
**Root Cause**: Tests create partial type literals instead of proper `SignalType` objects. The kernel function expects `SignalType` (which has `payload`, `unit`, `extent` axes), but tests only provide `{ payload, cardinality }`.

**Fix**: Use `signalTypeField(payload, instanceId)` which produces proper SignalType. For tests that don't need instance tracking, can create a test helper:
```typescript
...
**Root Cause**: Tests create mock block definitions with partial SignalType.

**Fix**: Use `signalType('float')` which returns complete SignalType with defaults:
```typescript
inputs: {
...
### Category 8: SignalType for local-space tests (6 instances)
**Status**: NOT_STARTED
