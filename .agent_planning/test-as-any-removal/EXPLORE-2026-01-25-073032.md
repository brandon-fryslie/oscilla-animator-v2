...
Root cause: The type parameter expects `SignalType` but tests are passing partial type literals.
Factory `signalType()` exists but produces different shape than expected by the kernel function.
...
Root cause: Incomplete SignalType literals passed to mock block definitions.
...
### Category 8: SignalType for field kernels (~6 instances)
...
### SignalType constructors (already exist):
- `signalType(payload: PayloadType): SignalType` - src/core/canonical-types.ts
- `signalTypeField(payload: PayloadType): SignalType`
- `signalTypeSignal(payload: PayloadType): SignalType`

### What's missing:
1. Test helper for creating proper SignalType for field kernels
