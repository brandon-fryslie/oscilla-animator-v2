...
### Blocks Using signalType('vec2')

| File | Line | Context |
|------|------|---------|
| geometry-blocks.ts | 34 | PolarToCartesian output: `pos: { label: 'Position', type: signalType('vec2') }` |
| geometry-blocks.ts | 55 | PolarToCartesian lower: `ctx.b.sigZip(inputs, polarFn, signalType('vec2'))` |
| geometry-blocks.ts | 151 | OffsetPosition input: `posIn: { label: 'Position In', type: signalType('vec2') }` |
| geometry-blocks.ts | 157 | OffsetPosition output: `posOut: { label: 'Position Out', type: signalType('vec2') }` |
| geometry-blocks.ts | 177 | OffsetPosition lower: `ctx.b.sigZip(inputs, offsetFn, signalType('vec2'))` |
| signal-blocks.ts | 128 | Const vec2 lowering: `sigId = ctx.b.sigZip([xSig, ySig], packFn, signalType('vec2'))` |
| field-operations-blocks.ts | 420 | FieldCartesianToPolar: `ctx.b.sigMap(pos.id, extractFn, signalType('vec2'))` - intermediate extraction |
...
| field-operations-blocks.ts | FieldPolarToCartesian | pos output: signalType('vec3') |
| field-operations-blocks.ts | FieldCartesianToPolar | pos input: signalType('vec3') |
| field-operations-blocks.ts | JitterVec | pos input/output: signalType('vec3') |
| field-operations-blocks.ts | SetZ | pos input, out output: signalType('vec3') |
...
- Line 768: Error message explicitly says "pos2→pos3 adapter" is NOT auto-promoted
