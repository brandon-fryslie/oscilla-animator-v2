...
show_help() {
  cat <<'HELP'
Usage: ./scripts/get-screenshot-of-compiler-tester.sh <fixture-name> [options]

Captures a canvas-only screenshot of a compiler tester fixture.
...
Examples:
  ./scripts/get-screenshot-of-compiler-tester.sh spinning-ring
  ./scripts/get-screenshot-of-compiler-tester.sh spinning-ring --output ./evidence/
  ./scripts/get-screenshot-of-compiler-tester.sh spinning-ring --no-headless
HELP
  exit 0
}
...
if [[ -z "$FIXTURE_NAME" ]]; then
  echo "${RED}Error:${RESET} Fixture name required. Example: ./scripts/get-screenshot-of-compiler-tester.sh spinning-ring" >&2
  exit 1
fi
