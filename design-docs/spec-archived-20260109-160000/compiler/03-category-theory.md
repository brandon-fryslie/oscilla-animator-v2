A few things you should know, and one big warning: you can absolutely use the useful parts of category-theory thinking here, but you should not surface it as “CT features” in the implementation spec, because engineers will cargo-cult jargon instead of enforcing invariants.
...
•	Auto-derive Signal and Field variants via map at lowering time by selecting the right evaluator:
...
If you want mixed-world arithmetic, define one canonical strategy:
...
•	Field and Signal values are data, not functions, at runtime.
...
TS can’t express real HKTs ergonomically without heavy type-level tricks. You can still model the concept structurally:
...
Use TS only to validate, and store the authoritative resolved types in your IR.
...
If a block is truly generic over T, it must not do anything that depends on the representation of T. That means:
