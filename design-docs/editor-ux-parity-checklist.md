...
| 10 | ExpressionEditorWorkbench / SharedExpressionEditor | **`.12`** | **Disposition resolved (2026-07-11, user directive "I want the full expression block"): SHIPPED, not wontfix.** The pillar path gains a native Expression block — new epic `oscilla-scene-expression-g86w` (`.1` DSL→PlanExpr compiler, `.2` the scalar Expression scene block + `'expression'` config control). `.12` is now the editor half — reseat the reusable `TokenExpressionEditor` leaf on the scene block via the existing neutral `ConfigField{kind:'expression'}` seam — and is **blocked by `g86w.2`** (the editor is a projection of the block; functionality precedes its UI). Scope = scalar Expression now; the field/bundle form (per-instance, multi-field) is `oscilla-scene-fields-by04.8`, blocked on the field-representation probe `by04.1`. |
