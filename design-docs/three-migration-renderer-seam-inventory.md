...
| --- | --- | --- | --- |
| Current compiler output for the old backend | `src/pillars/compile.ts`, `src/pillars/assembly/legacy-payload.ts`, `src/pillars/block-api.ts` | Freeze | Do not widen `PipelineInstallPayload` or Rust-specific manifest shapes as the main migration path. They describe the old backend contract. |
...
| Renderer stub + WebGPU renderer seam | `src/render/webgpu/index.ts` | Adapter boundary | Keep renderer construction local to the renderer module. Do not let browser GPU capability details leak back into authored graph or runtime policy modules. |
| Rust worker boundary | Deleted on 2026-04-12 (`src/render/rust/engine.worker.ts`, `src/render/rust/worker-protocol.ts`) | Removed | `// [LAW:one-way-deps]` The live app shell no longer carries a dedicated Rust worker message boundary as an active surface. |
| Legacy payload schema | `src/legacy/pipeline-install-contract.ts` | Freeze | `// [LAW:one-source-of-truth]` It remains the one source for the legacy payload contract, but it must stop being treated as the future renderer contract. |
| Rust WASM implementation | Deleted on 2026-04-13 (`src/render/wasm/rust/oscilla-rust-renderer`, `.github/workflows/rust-wasm.yaml`) after the active runtime wrapper had already been removed on 2026-04-12 | Removed | `// [LAW:locality-or-seam]` The old WASM renderer path is no longer part of the live tree, which keeps the new renderer seam local to `src/render/webgpu/`. |
| GPU-IR utilities | `src/render/gpu-ir` | Freeze, delete-later | Valuable as historical/legacy coverage, but not the canonical implementation direction once Three migration starts. |
| Shape-bank specific WebGPU helpers | Deleted on 2026-04-12 (`src/render/webgpu/ShapeBankGeometrySeam.ts`, `src/render/webgpu/WebGPUShapeBankManager.ts`) | Removed | `// [LAW:locality-or-seam]` These helpers were backend-local legacy shape-bank consumers, so deleting them shrinks the old path without touching app-facing seams. |
| Legacy manual test shells | Deleted on 2026-04-12 (`src/compiler-tester`, `src/payload-tester`, related Vite/public/scripts hooks) | Removed | `// [LAW:one-source-of-truth]` The app shell no longer advertises old-boundary manual harnesses as first-class runtime surfaces. |
| Legacy boundary dump tooling | Deleted on 2026-04-12 (`src/services/runtime-boundary-dump-bridge.ts`, `src/ui/components/app/boundary-fixture-copy.ts`) | Removed | `// [LAW:behavior-not-structure]` Old install/frame payload export helpers were preserving obsolete boundary structure rather than a surviving app contract. |
...
If implementation work needs to add new fields to `src/legacy/pipeline-install-contract.ts` first, it is probably violating the migration direction captured here.
