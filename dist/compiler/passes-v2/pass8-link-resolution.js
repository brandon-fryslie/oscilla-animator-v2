 ...
 */
export function pass8LinkResolution(fragments, blocks, edges) {
    ...
    const blockInputRoots = buildBlockInputRoots(blocks, blockOutputs, builder, errors, edges);
...
function registerFieldSlots(builder, blockOutputRoots) {
 ...
 */
function applyCameraLowering(builder, blocks, blockInputRoots, blockOutputRoots, errors) {
    ...
    for (let blockIdx = 0; blockIdx < blocks.length; blockIdx++) {
        ...
        for (const inputDecl of decl.inputs) {
            ...
            const ref = blockInputRoots.refs[blockInputRoots.indexOf(blockIdx, portIdx)];
            if (ref === undefined) {
        ...
        if (result.outputs.length > 0) {
            for (let outIdx = 0; outIdx < result.outputs.length; outIdx++) {
                ...
                if (outputDecl !== undefined) {
                    const idx = blockOutputRoots.indexOf(blockIdx, outIdx);
    ...
    for (let blockIdx = 0; blockIdx < blocks.length; blockIdx++) {
        ...
        if (decl !== undefined && decl.capability === "render") {
            console.warn(`[Pass 8] Render block "${block.type}" (${block.id}) encountered in pass8. ` +
 ...
 */
function buildBlockOutputRoots(blocks, blockOutputs) {
    const refs = [];
    ...
    return {
        refs,
        indexOf: (blockIndex, portIdx) => {
            return blockIndex * maxOutputs + portIdx;
        },
    };
}
// =============================================================================
 ...
 */
function getEdgeTransforms(edge) {
 ...
 */
function applyTransformStepIR(valueRef, step, builder, errors, context) {
    ...
    const transformId = isLens ? step.lens.lensId : step.adapterId ?? step.adapter;
 ...
 */
function applyTransformsIR(valueRef, transforms, builder, errors, context) {
    let result = valueRef;
 ...
 */
function buildBlockInputRoots(blocks, blockOutputs, builder, errors, edges) {
    const refs = [];
    ...
    for (let blockIdx = 0; blockIdx < blocks.length; blockIdx++) {
        ...
        for (let portIdx = 0; portIdx < blockDef.inputs.length; portIdx++) {
            ...
            const edge = edges.find((e) => e.to.kind === 'port' && e.to.blockId === block.id && e.to.slotId === input.id);
            if (edge !== undefined) {
                ...
                if (ref !== undefined) {
                    ...
                    if (transforms.length > 0) {
                        ref = applyTransformsIR(ref, transforms, builder, errors, `edge to ${block.type}.${input.id}`);
                    }
                    refs[flatIdx] = ref;
                    resolved = true;
                    continue;
    ...
    return {
        refs,
        indexOf: (blockIndex, portIdx) => {
            return blockIndex * maxInputs + portIdx;
        },
    };
}
/**
 ...
 */
function validateOutputSlots(blocks, blockOutputRoots, builder, errors) {
    ...
    for (let blockIdx = 0; blockIdx < blocks.length; blockIdx++) {
        ...
        for (let portIdx = 0; portIdx < blockDef.outputs.length; portIdx++) {
            ...
            const idx = blockOutputRoots.indexOf(blockIdx, portIdx);
