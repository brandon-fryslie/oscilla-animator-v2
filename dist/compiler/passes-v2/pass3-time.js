 ...
 */
function findTimeRoot(blocks, errors) {
    const timeRoots = [];
    ...
    if (timeRoots.length === 0) {
        ...
        return { id: '__missing__', type: 'FiniteTimeRoot', label: '', params: {}, position: { x: 0, y: 0 }, form: 'primitive' };
    }
 ...
 */
function extractTimeModel(timeRoot, errors) {
    // For FiniteTimeRoot blocks
    if (timeRoot.type === "FiniteTimeRoot") {
        const durationMs = timeRoot.params?.durationMs;
    ...
    if (timeRoot.type === "InfiniteTimeRoot") {
        const periodMs = timeRoot.params?.periodMs;
        const period = periodMs ?? 4000; // Default 4s loop
        const mode = timeRoot.params?.mode ?? "loop";
        if (period <= 0) {
    ...
    const periodMs = timeRoot.params?.periodMs;
    const duration = timeRoot.params?.duration;
    const topology = timeRoot.params?.topology;
 ...
 */
function generateTimeSignals(timeModel) {
 ...
 */
export function pass3TimeTopology(typedPatch) {
    const errors = [];
    ...
    if (errors.length > 0) {
        ...
        throw new Error(`Pass 3 (Time Topology) failed with ${errors.length} error(s):\n${errorSummary}`);
    }
