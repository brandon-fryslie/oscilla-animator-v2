...
import { createCombineNode, validateCombineMode, validateCombinePolicy, shouldCombine, } from "./combine-utils";
/**
 ...
 */
function resolveInputsWithMultiInput(block, edges, builder, errors, blockOutputs, blockIdToIndex) {
    ...
    for (const [slotId, spec] of resolved.entries()) {
        ...
        if (combine.mode !== 'error' && portType.world !== 'event' && isCoreDomain(portType.domain)) {
            const modeValidation = validateCombineMode(combine.mode, portType.world, portType.domain);
        ...
        const combinedRef = createCombineNode(combine.mode, writerRefs, portType, builder);
        if (combinedRef === null) {
            errors.push({
                code: 'NotImplemented',
 ...
 */
function getWriterValueRef(writer, errors, blockOutputs, blockIdToIndex) {
    if (writer.kind === 'wire') {
 ...
 */
function lowerBlockInstance(block, blockIndex, builder, errors, edges, blockOutputs, blockIdToIndex) {
    ...
    try {
        ...
        if (enforcePortContract && blockDef !== undefined) {
            ...
            if (inputOrderMismatch || outputOrderMismatch) {
                errors.push({
                    ...
                    message: `IR port contract mismatch for "${block.type}" (${block.id}). ` +
        ...
        const inputsById = edges !== undefined
 ...
 */
export function pass6BlockLowering(validated, blocks, edges) {
    ...
    for (let i = 0; i < blocks.length; i++) {
        blockIdToIndex.set(blocks[i].id, i);
    }
    ...
    for (const scc of validated.sccs) {
        for (const node of scc.nodes) {
            ...
            const outputRefs = lowerBlockInstance(block, blockIndex, builder, errors, edges, blockOutputs, blockIdToIndex);
