 ...
 */
export function validateCombineMode(mode, world, domain) {
    ...
    const numericDomains = ['float', 'int', 'vec2', 'vec3'];
 ...
 * @returns Validation result with reason if invalid
 */
export function validateCombinePolicy(policy, writerCount) {
 ...
 */
export function shouldCombine(policy, writerCount) {
 ...
 */
export function createCombineNode(mode, inputs, type, builder) {
    ...
    if (type.world === "event") {
        ...
        const eventMode = normalizedMode === 'last' ? 'last' : 'merge';
 ...
 */
export function sortEdgesBySortKey(edges) {
