...
import { registerBlock, portId, fieldType, sigType, } from '../registry';
const lowerFieldGoldenAngle = ({ b, inputsById, config }) => {
    ...
    const goldenAngleTimesNumTurns = b.sigConst(goldenAngle * numTurns, sigType('float'));
    const goldenField = b.fieldBroadcast(goldenAngleTimesNumTurns, fieldType('float'));
    const angle = b.fieldZip([id01.id, goldenField], { kind: 'opcode', opcode: OpCode.Mul }, fieldType('float'));
    return {
