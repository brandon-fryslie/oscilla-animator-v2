 ...
 */
export function materialize(fieldId, domainId, fields, signals, domains, state, pool) {
    // Check cache
 ...
 */
function fillBuffer(expr, buffer, domain, fields, signals, domains, state, pool) {
    ...
    switch (expr.kind) {
        ...
        case 'map': {
            ...
            const input = materialize(expr.input, domain.id, fields, signals, domains, state, pool);
            applyMap(buffer, input, expr.fn, N, expr.type);
            break;
        }
        case 'zip': {
            // Zip: get inputs, apply function
            const inputs = expr.inputs.map((id) => materialize(id, domain.id, fields, signals, domains, state, pool));
            applyZip(buffer, inputs, expr.fn, N, expr.type);
            break;
        }
        case 'zipSig': {
            // ZipSig: combine field with signals
            const fieldInput = materialize(expr.field, domain.id, fields, signals, domains, state, pool);
        ...
        default: {
            ...
            throw new Error(`Unknown field expr kind: ${_exhaustive.kind}`);
        }
    }
}
/**
 ...
 */
function fillBufferSource(sourceId, buffer, domain) {
    const N = domain.count;
    switch (sourceId) {
        case 'index': {
            const arr = buffer;
        ...
        case 'pos0': {
            ...
            if (domain.kind === 'grid') {
                const rows = domain.params.rows || 1;
                const cols = domain.params.cols || 1;
                const arr = buffer;
                for (let i = 0; i < N; i++) {
 ...
 */
function applyMap(out, input, fn, N, type) {
    const outArr = out;
    const inArr = input;
    if (fn.kind === 'opcode') {
        const op = fn.opcode;
 ...
 */
function applyZip(out, inputs, fn, N, type) {
    if (fn.kind === 'opcode') {
        const outArr = out;
        const inArrs = inputs.map((buf) => buf);
        const op = fn.opcode;
        for (let i = 0; i < N; i++) {
 ...
 */
function applyZipSig(out, fieldInput, sigValues, fn, N, type) {
    const outArr = out;
    const inArr = fieldInput;
    if (fn.kind === 'opcode') {
        const op = fn.opcode;
        for (let i = 0; i < N; i++) {
 ...
 */
function applyMapIndexed(out, fn, sigValues, N, domain, type) {
    const outArr = out;
    if (fn.kind === 'kernel') {
 ...
 */
function applyKernel(out, inputs, kernelName, N, type) {
    if (kernelName === 'makeVec2') {
        ...
        const outArr = out;
        const posArr = inputs[0];
        const randArr = inputs[1];
        const amountXArr = inputs[2];
        const amountYArr = inputs[3];
        for (let i = 0; i < N; i++) {
        ...
        const outArr = out;
        const posArr = inputs[0];
        const targetXArr = inputs[1];
        const targetYArr = inputs[2];
        const phaseArr = inputs[3];
        const strengthArr = inputs[4];
        for (let i = 0; i < N; i++) {
 ...
 */
function applyKernelZipSig(out, fieldInput, sigValues, kernelName, N, type) {
