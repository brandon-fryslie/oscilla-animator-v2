/**
 * tests/e2e/webgpu/three-gradient.spec.ts
 *
 * Visual proof for the native Gradient color source (oscilla-pillars-scene-nt56.21).
 * Boots the app shell, loads the authored `instance-gradient` patch
 * (`?scenePlan=instance-gradient`), and proves the preview canvas shows visible,
 * time-animated content rendered through the Three backend — the grid colored by
 * a perceptual OKLab gradient ramping across rank, with no legacy GPU-IR / Rust
 * path engaged.
 *
 * The colour ramp itself is confirmed by eye from the captured frame; this spec
 * guards the boot + render + animation contract. The PNG-decode/frame-diff
 * machinery is shared with the other steel-thread proofs. // [LAW:one-source-of-truth]
 *
...
const SELECTED_PATCH_ID = 'instance-gradient';
const ARTIFACT_DIR = resolve('artifacts/three-migration/nt56.21-gradient');
const FRAME_INTERVAL_MS = 400;
const BLACK_CHANNEL_THRESHOLD = 8;

test.describe('Three native Gradient color source', () => {
  test('renders the grid colored by an OKLab gradient through the Three backend, animated', async ({ page }) => {
    ...
    writeFileSync(
      ...
      `${JSON.stringify(
        {
          ticket: 'oscilla-pillars-scene-nt56.21',
