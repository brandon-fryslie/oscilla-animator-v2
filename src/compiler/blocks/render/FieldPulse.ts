...
const lowerFieldPulse: BlockLower = ({ b, inputsById }) => {
  ...
  const totalPhase = b.fieldZip(
    [phaseField, phaseOffset],
  ...
  const radians = b.fieldZip(
    [totalPhase, twoPiField],
    { kind: 'opcode', opcode: OpCode.Mul },
    fieldType('float')
  );

  // sin(radians)
  const sinValue = b.fieldMap(
    radians,
  ...
  const scaled = b.fieldZip(
    [sinValue, amplitudeField],
    { kind: 'opcode', opcode: OpCode.Mul },
    fieldType('float')
  );
