...
const lowerFieldPolarToCartesian: BlockLower = ({ b, inputsById }) => {
  ...
  // cos(angle), sin(angle)
  const cosAngle = b.fieldMap(
    angle.id,
    { kind: 'opcode', opcode: OpCode.Cos },
    fieldType('float')
  );
  const sinAngle = b.fieldMap(
    angle.id,
    { kind: 'opcode', opcode: OpCode.Sin },
    fieldType('float')
  );

  // x = centerX + radius * cos(angle)
  const xOffset = b.fieldZip(
    [radius.id, cosAngle],
  ...
  const x = b.fieldZip(
    [centerXField, xOffset],
    { kind: 'opcode', opcode: OpCode.Add },
    fieldType('float')
  );

  // y = centerY + radius * sin(angle)
  const yOffset = b.fieldZip(
    [radius.id, sinAngle],
    { kind: 'opcode', opcode: OpCode.Mul },
    fieldType('float')
  );
  ...
  const y = b.fieldZip(
    [centerYField, yOffset],
    { kind: 'opcode', opcode: OpCode.Add },
    fieldType('float')
  );

  // Combine x and y into vec2
  const pos = b.fieldZip(
    [x, y],
    { kind: 'kernel', name: 'makeVec2' },
    fieldType('vec2')
  );
