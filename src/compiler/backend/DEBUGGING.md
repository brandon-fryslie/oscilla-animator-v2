...
`DebugIndexIR.exprProvenance` maps each `ValueExprId` to an `ExprProvenanceIR` record containing only emitted block identity and output port name. The debugger resolves these emitted identities through `blockMap` + `blockDisplayNames` to render user-facing labels.
