            ...
            match entry {
                RosterEntry::Compute(spec) => {
                    ...
                    let shader = self.device.create_shader_module(wgpu::ShaderModuleDescriptor {
                        ...
                        source: wgpu::ShaderSource::Naga(std::borrow::Cow::Owned(compute_result.module)),
                    });
                    ...
                    console::log_1(&JsValue::from_str(&format!(
                        "[install_pipeline] Compute '{}': uses_globals={}, uses_scalars={}, domains={}, textures={}",
                        spec.pass_id, compute_result.uses_globals, compute_result.uses_scalars,
                        compute_result.bound_domain_keys.len(), compute_result.bound_texture_keys.len()
                    )));
                    ...
                    let auto_group0 = if has_group0 {
                        ...
                        if compute_result.uses_globals {
                            entries.push(wgpu::BindGroupEntry {
                                binding, resource: arena.globals_buffer.as_entire_binding(),
                            });
                            binding += 1;
                        }
                        if compute_result.uses_scalars {
                            entries.push(wgpu::BindGroupEntry {
                                binding, resource: arena.scalars_buffer.as_entire_binding(),
                            });
                        }
                    ...
                    let auto_group1 = if has_group1 {
                        ...
                        for domain_id in &compute_result.bound_domain_keys {
                            bg_entries.push(wgpu::BindGroupEntry {
                                binding, resource: arena.domain_buffers[domain_id].as_entire_binding(),
                            });
                            binding += 1;
                        }
                        // Textures next
