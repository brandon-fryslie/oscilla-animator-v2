...
use crate::contract::EnvironmentResizePayload;
...
#[wasm_bindgen]
pub fn update_data_stream(stream_id: &str, data: &[u8]) -> Result<(), JsValue> {
    ENGINE.with(|engine_cell| {
        let engine_ref = engine_cell.borrow();
        let engine = engine_ref.as_ref().ok_or_else(|| {
            JsValue::from_str("Rust engine must be initialized before update_data_stream")
        })?;
        engine.update_data_stream(stream_id, data)
    })
}

#[wasm_bindgen]
pub fn resize_environment(payload_json: &str) -> Result<(), JsValue> {
    let payload: EnvironmentResizePayload = serde_json::from_str(payload_json)
        .map_err(|error| JsValue::from_str(&format!("resize_environment parse error: {error}")))?;
    ENGINE.with(|engine_cell| {
        let mut engine_ref = engine_cell.borrow_mut();
        let engine = engine_ref.as_mut().ok_or_else(|| {
            JsValue::from_str("Rust engine must be initialized before resize_environment")
        })?;
        engine.resize_environment(payload.width, payload.height)
    })
}
