...
pub fn translate_compute_pass(
    ...
    ComputePassTranslation { module, info, bound_domain_keys, bound_texture_keys, uses_globals, uses_scalars }
}
...
fn stmt_uses_scalars(stmt: &StatementIR) -> bool {
    match stmt {
        ...
        StatementIR::StoreField { index, value, .. } => expr_uses_scalars(index) || expr_uses_scalars(value),
        StatementIR::If { condition, accept, reject } => {
            expr_uses_scalars(condition) || ast_uses_scalars(accept) || ast_uses_scalars(reject)
        }
        StatementIR::For { init, condition, update, body } => {
            stmt_uses_scalars(init) || expr_uses_scalars(condition) || stmt_uses_scalars(update) || ast_uses_scalars(body)
        }
        _ => false,
    }
}
...
fn expr_uses_scalars(expr: &ExprIR) -> bool {
    match expr {
        ...
        ExprIR::UnaryOp { expr: inner, .. } | ExprIR::Cast { expr: inner, .. } => expr_uses_scalars(inner),
        ExprIR::CallBuiltin { args, .. } | ExprIR::Construct { args, .. } => args.iter().any(|a| expr_uses_scalars(a)),
        _ => false,
    }
}
...
pub fn translate_render_pass(
    ...
    let vs_ctx = PassContext {
        globals_expr: None, scalars_expr: None,
    ...
    RenderPassTranslation { module, info, bound_domain_keys, bound_texture_keys }
}
...
fn translate_statement_body(
    ...
    match stmt {
        ...
        StatementIR::StoreField {
            ...
            let val = translate_expr_body(bb, ctx, value, scope);
            let store_val = if sym.wgsl_type == "u32" || sym.wgsl_type == "i32" || is_u32_expr(value) {
                val
            } else {
                bb.bitcast_u32(val)
            };
