...
use crate::contract::{
    ComputePassSpec, DrawCallSpec, ExprIR, StatementIR, SystemPassSpec,
};
...
struct PassContext {
    stage: ShaderTranslationStage,
    globals_expr: Option<Expr>,
    scalars_expr: Option<Expr>,
    atomic_scalars_expr: Option<Expr>,
    /// Domain ID → standard buffer expression handle
    domain_standard_exprs: HashMap<String, Expr>,
    /// Domain ID → atomic buffer expression handle
    domain_atomic_exprs: HashMap<String, Expr>,
    /// Stream ID → buffer expression handle
    data_stream_exprs: HashMap<String, Expr>,
...
impl TranslationScope {
    ...
    fn from_let_scope(let_scope: HashMap<String, Expr>) -> Self {
        Self { let_scope, var_scope: HashMap::new() }
    }
...
pub struct ComputePassTranslation {
    ...
    pub bound_stream_keys: Vec<String>,
    pub bound_texture_keys: Vec<String>,
    pub bound_sampler_keys: Vec<String>,
    pub uses_globals: bool,
    pub uses_scalars_standard: bool,
    pub uses_scalars_atomic: bool,
}
...
pub fn translate_compute_pass(
    ...
    let (uses_scalars_standard, uses_scalars_atomic) = ast_uses_scalars(&spec.ast, &arena.symbol_map);
    ...
    let globals_gv = if uses_globals {
        ...
        let gv = m.add_global_storage("globals", arr_ty, 0, group0_binding, naga::StorageAccess::LOAD);
        group0_binding += 1;
        Some(gv)
    } else {
        None
    };
    let scalars_gv = if uses_scalars_standard {
        let arr_ty = m.array_type(u32_ty, None, 4);
        let gv = m.add_global_storage("scalars", arr_ty, 0, group0_binding, naga::StorageAccess::LOAD | naga::StorageAccess::STORE);
        group0_binding += 1;
        Some(gv)
    } else {
        None
    };
    let atomic_scalars_gv = if uses_scalars_atomic {
        let atomic_u32_ty = m.atomic_type(naga::ScalarKind::Uint);
        let arr_ty = m.array_type(atomic_u32_ty, None, 4);
    ...
    let mut domain_keys: Vec<_> = spec.dependencies.domains.keys()
        .filter(|k| referenced.contains(*k))
        .collect();
    domain_keys.sort();
    let mut domain_standard_gvs = HashMap::new();
    let mut domain_atomic_gvs = HashMap::new();
    let mut group1_binding = 0u32;
    for domain_id in &domain_keys {
        ...
        let standard_gv = m.add_global_storage(
            domain_id,
            domain_arr_ty,
            1,
            group1_binding,
        ...
        domain_standard_gvs.insert((*domain_id).clone(), standard_gv);
        group1_binding += 1;
        if arena
            .domain_buffers
            .get(*domain_id)
            ...
            .is_some()
        {
            let atomic_gv = m.add_global_storage(
            ...
            domain_atomic_gvs.insert((*domain_id).clone(), atomic_gv);
            group1_binding += 1;
        }
    }

    // Group 1 continued: Data streams (sorted alpha, after domains)
    let referenced_streams = collect_streams_from_stmts_set(&spec.ast, arena);
    let mut stream_gvs = HashMap::new();
    let mut stream_keys: Vec<_> = spec
        .dependencies
        .data_streams
        .keys()
        .filter(|k| referenced_streams.contains(*k))
        .collect();
    stream_keys.sort();
    for stream_id in &stream_keys {
        let access_str = &spec.dependencies.data_streams[*stream_id];
        let access = if access_str == "read" {
            naga::StorageAccess::LOAD
        } else {
            naga::StorageAccess::LOAD | naga::StorageAccess::STORE
        };
        let stream_arr_ty = m.array_type(u32_ty, None, 4);
        let gv = m.add_global_storage(stream_id, stream_arr_ty, 1, group1_binding, access);
        stream_gvs.insert((*stream_id).clone(), gv);
        group1_binding += 1;
    }

    // Group 1 continued: Textures (sorted alpha, after domains/streams)
    let mut texture_gvs = HashMap::new();
    let mut tex_keys: Vec<_> = spec.dependencies.textures.keys().collect();
    tex_keys.sort();
    for texture_id in &tex_keys {
        let access_str = &spec.dependencies.textures[*texture_id];
        let tex_info = arena.textures.get(*texture_id);
        let format = tex_info.map(|t| t.format).unwrap_or(wgpu::TextureFormat::Rgba8Unorm);
        let dim = tex_info.map(|t| t.dimension).unwrap_or(wgpu::TextureViewDimension::D2);
        ...
        let class = match access_str.as_str() {
            "read" | "sampled" => naga::ImageClass::Sampled { kind: naga::ScalarKind::Float, multi: false },
    ...
    let mut sampler_keys: Vec<_> = spec.dependencies.samplers.keys().collect();
    sampler_keys.sort();
    ...
    let atomic_scalars_expr = atomic_scalars_gv.map(|gv| fb.global(gv));
    let mut domain_standard_exprs = HashMap::new();
    for (domain_id, gv) in &domain_standard_gvs {
        domain_standard_exprs.insert(domain_id.clone(), fb.global(*gv));
    }
    ...
    let mut data_stream_exprs = HashMap::new();
    for (stream_id, gv) in &stream_gvs {
        data_stream_exprs.insert(stream_id.clone(), fb.global(*gv));
    }
    ...
    type_handles.insert("vec2<f32>".into(), m.vector_type(naga::VectorSize::Bi, naga::ScalarKind::Float));
    type_handles.insert("vec2<i32>".into(), m.vector_type(naga::VectorSize::Bi, naga::ScalarKind::Sint));
    type_handles.insert("vec2<u32>".into(), m.vector_type(naga::VectorSize::Bi, naga::ScalarKind::Uint));
    type_handles.insert("vec3<f32>".into(), m.vector_type(naga::VectorSize::Tri, naga::ScalarKind::Float));
    type_handles.insert("vec3<u32>".into(), vec3_u32_ty);
    type_handles.insert("vec4<f32>".into(), m.vec4_f32_type());

    let ctx = PassContext {
        stage: ShaderTranslationStage::Compute,
        globals_expr,
        scalars_expr,
        atomic_scalars_expr,
        domain_standard_exprs,
        domain_atomic_exprs,
        data_stream_exprs,
    ...
    let bound_stream_keys: Vec<String> = stream_keys.into_iter().cloned().collect();
    let bound_texture_keys: Vec<String> = tex_keys.into_iter().cloned().collect();
    let bound_sampler_keys: Vec<String> = sampler_keys.into_iter().cloned().collect();
...
pub fn translate_draw_prep(
    ...
    let scalars_gv = m.add_global_storage(
        "scalars",
        scalars_arr_ty,
        0,
        0,
...
fn stmt_uses_globals(stmt: &StatementIR) -> bool {
    match stmt {
        ...
        StatementIR::StoreField { index, value, .. } => expr_uses_globals(index) || expr_uses_globals(value),
        ...
        StatementIR::Assign { target, value } => expr_uses_globals(target) || expr_uses_globals(value),
        StatementIR::If { condition, accept, reject } => {
            expr_uses_globals(condition) || ast_uses_globals(accept) || ast_uses_globals(reject)
        }
        StatementIR::For { init, condition, update, body } => {
            stmt_uses_globals(init) || expr_uses_globals(condition) || stmt_uses_globals(update) || ast_uses_globals(body)
        }
...
fn expr_uses_globals(expr: &ExprIR) -> bool {
    match expr {
        ...
        ExprIR::UnaryOp { expr: inner, .. } | ExprIR::Cast { expr: inner, .. } => expr_uses_globals(inner),
        ExprIR::CallBuiltin { args, .. } | ExprIR::Construct { args, .. } => args.iter().any(|a| expr_uses_globals(a)),
        ExprIR::Swizzle { source, .. } => expr_uses_globals(source),
        ExprIR::LoadField { index, .. } => expr_uses_globals(index),
        ExprIR::IndexAccess { target, index } => expr_uses_globals(target) || expr_uses_globals(index),
        _ => false,
    }
}

/// Check scalar usage split between standard and atomic arena scalar buffers.
fn ast_uses_scalars(
    stmts: &[StatementIR],
    symbol_map: &HashMap<String, PhysicalSymbol>,
) -> (bool, bool) {
    let mut uses_standard = false;
    let mut uses_atomic = false;
    for stmt in stmts {
        stmt_uses_scalars(stmt, symbol_map, &mut uses_standard, &mut uses_atomic);
    }
    (uses_standard, uses_atomic)
}

fn mark_scalar_symbol(
    symbol_id: &str,
    symbol_map: &HashMap<String, PhysicalSymbol>,
    uses_standard: &mut bool,
    uses_atomic: &mut bool,
) {
    if let Some(sym) = symbol_map.get(symbol_id) {
        match sym.buffer_kind {
            BufferKind::ArenaScalar => *uses_standard = true,
            BufferKind::ArenaScalarAtomic => *uses_atomic = true,
            _ => {}
        }
    }
}

fn stmt_uses_scalars(
    stmt: &StatementIR,
    symbol_map: &HashMap<String, PhysicalSymbol>,
    uses_standard: &mut bool,
    uses_atomic: &mut bool,
    ...
    match stmt {
        StatementIR::StoreScalar { symbol_id, value } => {
            mark_scalar_symbol(symbol_id, symbol_map, uses_standard, uses_atomic);
            expr_uses_scalars(value, symbol_map, uses_standard, uses_atomic);
        }
        StatementIR::AtomicOpScalar { symbol_id, value, .. } => {
            mark_scalar_symbol(symbol_id, symbol_map, uses_standard, uses_atomic);
            expr_uses_scalars(value, symbol_map, uses_standard, uses_atomic);
        }
        ...
        StatementIR::Assign { target, value } => {
            expr_uses_scalars(target, symbol_map, uses_standard, uses_atomic);
            expr_uses_scalars(value, symbol_map, uses_standard, uses_atomic);
        }
        StatementIR::StoreField { index, value, .. } => {
            expr_uses_scalars(index, symbol_map, uses_standard, uses_atomic);
            expr_uses_scalars(value, symbol_map, uses_standard, uses_atomic);
        }
        StatementIR::If { condition, accept, reject } => {
            expr_uses_scalars(condition, symbol_map, uses_standard, uses_atomic);
            for nested in accept {
                stmt_uses_scalars(nested, symbol_map, uses_standard, uses_atomic);
            }
            for nested in reject {
                stmt_uses_scalars(nested, symbol_map, uses_standard, uses_atomic);
            }
        }
        StatementIR::For { init, condition, update, body } => {
            stmt_uses_scalars(init, symbol_map, uses_standard, uses_atomic);
            expr_uses_scalars(condition, symbol_map, uses_standard, uses_atomic);
            stmt_uses_scalars(update, symbol_map, uses_standard, uses_atomic);
            for nested in body {
                stmt_uses_scalars(nested, symbol_map, uses_standard, uses_atomic);
            }
        }
        ...
        StatementIR::TextureStore { coords, value, .. } => {
            expr_uses_scalars(coords, symbol_map, uses_standard, uses_atomic);
            expr_uses_scalars(value, symbol_map, uses_standard, uses_atomic);
        }
        StatementIR::ReturnVertex { position, varyings } => {
            expr_uses_scalars(position, symbol_map, uses_standard, uses_atomic);
            for value in varyings.values() {
                expr_uses_scalars(value, symbol_map, uses_standard, uses_atomic);
            }
        }
...
fn expr_uses_scalars(
    ...
    match expr {
        ExprIR::LoadScalar { symbol_id } | ExprIR::AtomicLoadScalar { symbol_id } => {
            mark_scalar_symbol(symbol_id, symbol_map, uses_standard, uses_atomic);
        }
        ExprIR::BinaryOp { left, right, .. } => {
            expr_uses_scalars(left, symbol_map, uses_standard, uses_atomic);
            expr_uses_scalars(right, symbol_map, uses_standard, uses_atomic);
        }
        ExprIR::UnaryOp { expr: inner, .. } | ExprIR::Cast { expr: inner, .. } => {
            expr_uses_scalars(inner, symbol_map, uses_standard, uses_atomic);
        }
        ExprIR::CallBuiltin { args, .. } | ExprIR::Construct { args, .. } => {
            for arg in args {
                expr_uses_scalars(arg, symbol_map, uses_standard, uses_atomic);
            }
        }
        ExprIR::Swizzle { source, .. } => {
            expr_uses_scalars(source, symbol_map, uses_standard, uses_atomic);
        }
        ...
        ExprIR::TextureSample { uv, .. } => {
            expr_uses_scalars(uv, symbol_map, uses_standard, uses_atomic);
        }
        ExprIR::TextureLoad { coords, .. } => {
            expr_uses_scalars(coords, symbol_map, uses_standard, uses_atomic);
        }
        ExprIR::LoadGlobal { .. }
        | ExprIR::Intrinsic { .. }
...
fn collect_streams_from_stmts_set(
    stmts: &[StatementIR],
    arena: &GpuMemoryArena,
) -> std::collections::HashSet<String> {
    let mut streams = std::collections::HashSet::new();
    collect_streams_from_stmts(stmts, arena, &mut streams);
    streams
}

fn collect_referenced_streams(
    vertex_ast: &[StatementIR],
    fragment_ast: &[StatementIR],
    arena: &GpuMemoryArena,
) -> std::collections::HashSet<String> {
    let mut streams = std::collections::HashSet::new();
    collect_streams_from_stmts(vertex_ast, arena, &mut streams);
    collect_streams_from_stmts(fragment_ast, arena, &mut streams);
    streams
}

fn collect_streams_from_stmts(
    stmts: &[StatementIR],
    arena: &GpuMemoryArena,
    out: &mut std::collections::HashSet<String>,
) {
    for stmt in stmts {
        collect_streams_from_stmt(stmt, arena, out);
    }
}

fn collect_streams_from_stmt(
    stmt: &StatementIR,
    arena: &GpuMemoryArena,
    out: &mut std::collections::HashSet<String>,
) {
    match stmt {
        StatementIR::Let { value, .. } => collect_streams_from_expr(value, arena, out),
        StatementIR::Var { value, .. } => {
            if let Some(value) = value {
                collect_streams_from_expr(value, arena, out);
            }
        }
        StatementIR::Assign { target, value } => {
            collect_streams_from_expr(target, arena, out);
            collect_streams_from_expr(value, arena, out);
        }
        StatementIR::StoreField { symbol_id, index, value } => {
            maybe_collect_stream_symbol(symbol_id, arena, out);
            collect_streams_from_expr(index, arena, out);
            collect_streams_from_expr(value, arena, out);
        }
        StatementIR::AtomicOpField { symbol_id, index, value, .. } => {
            maybe_collect_stream_symbol(symbol_id, arena, out);
            collect_streams_from_expr(index, arena, out);
            collect_streams_from_expr(value, arena, out);
        }
        ...
        StatementIR::ReturnVertex { position, varyings } => {
            collect_streams_from_expr(position, arena, out);
            for value in varyings.values() {
                collect_streams_from_expr(value, arena, out);
            }
        }
        ...
        StatementIR::If { condition, accept, reject } => {
            collect_streams_from_expr(condition, arena, out);
            collect_streams_from_stmts(accept, arena, out);
            collect_streams_from_stmts(reject, arena, out);
        }
        StatementIR::For { init, condition, update, body } => {
            collect_streams_from_stmt(init, arena, out);
            collect_streams_from_expr(condition, arena, out);
            collect_streams_from_stmt(update, arena, out);
            collect_streams_from_stmts(body, arena, out);
        }
        _ => {}
    }
}

fn collect_streams_from_expr(
    expr: &ExprIR,
    arena: &GpuMemoryArena,
    out: &mut std::collections::HashSet<String>,
) {
    match expr {
        ExprIR::LoadField { symbol_id, index } | ExprIR::AtomicLoadField { symbol_id, index } => {
            maybe_collect_stream_symbol(symbol_id, arena, out);
            collect_streams_from_expr(index, arena, out);
        }
        ExprIR::AtomicLoadScalar { symbol_id } | ExprIR::LoadScalar { symbol_id } | ExprIR::LoadGlobal { symbol_id } => {
            maybe_collect_stream_symbol(symbol_id, arena, out);
        }
        ExprIR::BinaryOp { left, right, .. } => {
            collect_streams_from_expr(left, arena, out);
            collect_streams_from_expr(right, arena, out);
        }
        ExprIR::UnaryOp { expr, .. } | ExprIR::Cast { expr, .. } | ExprIR::Swizzle { source: expr, .. } => {
            collect_streams_from_expr(expr, arena, out);
        }
        ExprIR::CallBuiltin { args, .. } | ExprIR::Construct { args, .. } => {
            for arg in args {
                collect_streams_from_expr(arg, arena, out);
            }
        }
        ExprIR::IndexAccess { target, index } => {
            collect_streams_from_expr(target, arena, out);
        ...
        ExprIR::TextureSample { uv, .. } => collect_streams_from_expr(uv, arena, out),
        ExprIR::TextureLoad { coords, .. } => collect_streams_from_expr(coords, arena, out),
        _ => {}
    }
}

fn maybe_collect_stream_symbol(
    symbol_id: &str,
    arena: &GpuMemoryArena,
    out: &mut std::collections::HashSet<String>,
) {
    let Some(symbol) = arena.symbol_map.get(symbol_id) else {
        return;
    };
    if symbol.buffer_kind == BufferKind::DataStream {
        if let Some(stream_id) = &symbol.domain_id {
            out.insert(stream_id.clone());
        }
    }
}
    ...
    match stmt {
        ...
        StatementIR::StoreField { symbol_id, index, value, .. } => {
            if let Some(sym) = arena.symbol_map.get(symbol_id) {
                if let Some(ref d) = sym.domain_id { out.insert(d.clone()); }
            }
        ...
        StatementIR::AtomicOpField { symbol_id, index, value, .. } => {
            if let Some(sym) = arena.symbol_map.get(symbol_id) {
                if sym.buffer_kind == BufferKind::DomainStandard || sym.buffer_kind == BufferKind::DomainAtomic {
        ...
        StatementIR::ReturnVertex { position, varyings } => {
            ...
            for v in varyings.values() { collect_domains_from_expr(v, arena, out); }
        }
        StatementIR::ReturnFragment { outputs } => {
            for v in outputs.values() { collect_domains_from_expr(v, arena, out); }
        }
        StatementIR::If { condition, accept, reject } => {
        ...
        _ => {} // Other statements don't reference domains in render passes
    }
}
...
fn collect_domains_from_expr(
    ...
    match expr {
        ...
        ExprIR::CallBuiltin { args, .. } | ExprIR::Construct { args, .. } => {
            for a in args { collect_domains_from_expr(a, arena, out); }
        }
...
pub fn translate_render_pass(
    ...
    // Group 0: domains + streams + textures + samplers (only referenced IDs)
    let referenced_domains = collect_referenced_domains(
        &draw_call.vertex_ast,
        &draw_call.fragment_ast,
        arena,
    );
    let referenced_streams = collect_referenced_streams(
        &draw_call.vertex_ast,
        &draw_call.fragment_ast,
        arena,
    );
    ...
    for domain_id in &domain_keys {
        let domain_arr_ty = m.array_type(u32_ty, None, 4);
        let standard_gv = m.add_global_storage(
            domain_id, domain_arr_ty, 0, group0_binding,
            naga::StorageAccess::LOAD,
        );
        domain_standard_gvs.insert((*domain_id).clone(), standard_gv);
        group0_binding += 1;
    ...
    let mut stream_gvs = HashMap::new();
    let mut stream_keys: Vec<_> = draw_call
        .dependencies
        .data_streams
        .keys()
        .filter(|k| referenced_streams.contains(*k))
    ...
    let mut render_tex_keys: Vec<_> = draw_call.dependencies.textures.keys().collect();
    ...
    for texture_id in &render_tex_keys {
        ...
        let class = match access_str.as_str() {
            "sampled" => naga::ImageClass::Sampled { kind: naga::ScalarKind::Float, multi: false },
            _ => naga::ImageClass::Storage {
                format: wgpu_format_to_naga(format),
    ...
    // Group 0 continued: Samplers (sorted alpha)
    let mut render_sampler_gvs = HashMap::new();
    let mut render_sampler_keys: Vec<_> = draw_call.dependencies.samplers.keys().collect();
    ...
    if let Some(struct_ty) = vs_output_ty {
        ...
        vs.set_result(vec4_f32_ty, Some(naga::Binding::BuiltIn(naga::BuiltIn::Position { invariant: false })));
    }

    let position_arg = vs.add_argument("position", vec2_f32_ty, Some(naga::Binding::Location {
        location: 0, blend_src: None, per_primitive: false,
        interpolation: Some(naga::Interpolation::Perspective),
        sampling: Some(naga::Sampling::Center),
    }));
    let instance_index_arg = vs.add_argument("instance_index", u32_ty, Some(naga::Binding::BuiltIn(naga::BuiltIn::InstanceIndex)));
    let vertex_index_arg = vs.add_argument("vertex_index", u32_ty, Some(naga::Binding::BuiltIn(naga::BuiltIn::VertexIndex)));
    ...
    let mut vs_domain_standard_exprs = HashMap::new();
    for (domain_id, gv) in &domain_standard_gvs {
        vs_domain_standard_exprs.insert(domain_id.clone(), vs.global(*gv));
    }
    let mut vs_domain_atomic_exprs = HashMap::new();
    for (domain_id, gv) in &domain_atomic_gvs {
        vs_domain_atomic_exprs.insert(domain_id.clone(), vs.global(*gv));
    }
    let mut vs_stream_exprs = HashMap::new();
    for (stream_id, gv) in &stream_gvs {
        vs_stream_exprs.insert(stream_id.clone(), vs.global(*gv));
    }
    let mut vs_texture_exprs = HashMap::new();
    for (tex_id, gv) in &render_texture_gvs {
        vs_texture_exprs.insert(tex_id.clone(), vs.global(*gv));
    }
    let mut vs_sampler_exprs = HashMap::new();
    for (sampler_id, gv) in &render_sampler_gvs {
        vs_sampler_exprs.insert(sampler_id.clone(), vs.global(*gv));
    }
    let vs_ctx = PassContext {
        stage: ShaderTranslationStage::Vertex,
        globals_expr: None,
        scalars_expr: None,
        atomic_scalars_expr: None,
        domain_standard_exprs: vs_domain_standard_exprs,
        domain_atomic_exprs: vs_domain_atomic_exprs,
        data_stream_exprs: vs_stream_exprs,
        symbol_map: arena.symbol_map.clone(),
        type_handles: type_handles.clone(),
        texture_exprs: vs_texture_exprs,
        sampler_exprs: vs_sampler_exprs,
    };
    ...
    fs.set_result(vec4_f32_ty, Some(naga::Binding::Location {
        location: 0, blend_src: None, per_primitive: false,
        interpolation: None, sampling: None,
    }));
    ...
    let mut fs_domain_standard_exprs = HashMap::new();
    for (domain_id, gv) in &domain_standard_gvs {
        fs_domain_standard_exprs.insert(domain_id.clone(), fs.global(*gv));
    }
    let mut fs_domain_atomic_exprs = HashMap::new();
    for (domain_id, gv) in &domain_atomic_gvs {
        fs_domain_atomic_exprs.insert(domain_id.clone(), fs.global(*gv));
    }
    let mut fs_stream_exprs = HashMap::new();
    for (stream_id, gv) in &stream_gvs {
        fs_stream_exprs.insert(stream_id.clone(), fs.global(*gv));
    }
    ...
    let fs_ctx = PassContext {
        stage: ShaderTranslationStage::Fragment,
        globals_expr: None,
        scalars_expr: None,
        atomic_scalars_expr: None,
        domain_standard_exprs: fs_domain_standard_exprs,
        domain_atomic_exprs: fs_domain_atomic_exprs,
        data_stream_exprs: fs_stream_exprs,
        symbol_map: arena.symbol_map.clone(),
        type_handles,
    ...
    translate_statements_fragment(&mut fs, &fs_ctx, &draw_call.fragment_ast, &mut fs_scope, vec4_f32_ty, f32_ty);
    ...
    let bound_sampler_keys: Vec<String> = render_sampler_keys.into_iter().cloned().collect();
    ...
    let raw_module = m.finish();
    {
        ...
        if let Ok(info) = permissive.validate(&raw_module) {
            if let Ok(wgsl) = naga::back::wgsl::write_string(
                &raw_module, &info, naga::back::wgsl::WriterFlags::empty(),
            ) {
...
fn resolve_storage_buffer_expr<'a>(
    ctx: &'a PassContext,
    ...
    match symbol.buffer_kind {
        BufferKind::DomainStandard => {
            let domain_id = symbol.domain_id.as_ref().unwrap_or_else(|| {
                panic!("Symbol '{}' missing domain id for domain standard buffer", symbol_id)
            });
            *ctx.domain_standard_exprs.get(domain_id).unwrap_or_else(|| {
                panic!("Domain '{}' not bound in pass context for symbol '{}'", domain_id, symbol_id)
            })
        }
        BufferKind::DomainAtomic => {
            let domain_id = symbol.domain_id.as_ref().unwrap_or_else(|| {
                panic!("Symbol '{}' missing domain id for domain atomic buffer", symbol_id)
            });
            *ctx.domain_atomic_exprs.get(domain_id).unwrap_or_else(|| {
                panic!(
                    "Atomic domain '{}' not bound in pass context for symbol '{}'",
                    domain_id, symbol_id
                )
            })
        }
        BufferKind::DataStream => {
            let stream_id = symbol.domain_id.as_ref().unwrap_or_else(|| {
                panic!("Symbol '{}' missing stream id for data stream buffer", symbol_id)
            });
            *ctx.data_stream_exprs.get(stream_id).unwrap_or_else(|| {
                panic!("Data stream '{}' not bound for symbol '{}'", stream_id, symbol_id)
            })
        }
        _ => panic!(
            "Symbol '{}' references unsupported storage buffer kind {:?}",
            symbol_id, symbol.buffer_kind
        ),
    }
}

fn map_atomic_function(op: &str) -> naga::AtomicFunction {
    match op {
        ...
        _ => panic!("Unsupported atomic op '{}'", op),
    }
}
...
fn translate_statement_body(
    ...
    match stmt {
        ...
        StatementIR::Var { name, data_type, value } => {
            let ty = data_type
                .as_ref()
                .and_then(|dt| ctx.type_handles.get(dt.as_str()))
                .copied()
                .unwrap_or_else(|| {
                    *ctx.type_handles.get("f32").expect("f32 type handle missing")
                });
            let init = value.as_ref().map(|v| translate_expr_body(bb, ctx, v, scope));
        ...
        StatementIR::Assign { target, value } => {
            ...
            let ExprIR::VarRef { name } = target else {
                panic!("Assign target must be a VarRef, got {:?}", std::mem::discriminant(target));
            };
            let (ptr, _) = scope.resolve(name).unwrap_or_else(|| {
                panic!("Assign: variable '{}' not found in scope", name)
            });
        ...
        StatementIR::StoreScalar { symbol_id, value } => {
            let sym = ctx.symbol_map.get(symbol_id).unwrap_or_else(|| {
                panic!("StoreScalar: symbol '{}' not in symbol_map", symbol_id)
            });
            let val = translate_expr_body(bb, ctx, value, scope);
            let store_val = if is_u32_expr(value) { val } else { bb.bitcast_u32(val) };
            let offset = bb.lit_u32(sym.word_offset);
            match sym.buffer_kind {
                BufferKind::ArenaScalar => {
                    let scalars = ctx.scalars_expr.expect("StoreScalar requires scalars buffer");
                    bb.store_buffer(scalars, offset, store_val);
                }
                BufferKind::ArenaScalarAtomic => {
                    let scalars = ctx
                        .atomic_scalars_expr
                        .expect("StoreScalar requires atomic scalars buffer");
                    let pointer = bb.access(scalars, offset);
                    bb.atomic(
                        pointer,
                        naga::AtomicFunction::Exchange { compare: None },
                        store_val,
                        *ctx.type_handles.get("u32").expect("u32 missing"),
                    );
                }
                _ => {
                    panic!(
                        "StoreScalar symbol '{}' points to non-scalar buffer kind {:?}",
                        symbol_id, sym.buffer_kind
                    );
                }
            }
        }
        StatementIR::StoreField {
            ...
            let sym = ctx.symbol_map.get(symbol_id).unwrap_or_else(|| {
                panic!("StoreField: symbol '{}' not in symbol_map", symbol_id)
            });
            let buffer_expr = resolve_storage_buffer_expr(ctx, sym, symbol_id);
            let idx = translate_expr_body(bb, ctx, index, scope);
            let val = translate_expr_body(bb, ctx, value, scope);
            let store_val = if sym.is_atomic || sym.wgsl_type == "u32" || sym.wgsl_type == "i32" || is_u32_expr(value) {
                val
            } else {
                bb.bitcast_u32(val)
            };
            ...
            bb.store_buffer(buffer_expr, addr, store_val);
        }
        StatementIR::TextureStore { texture_id, coords, value } => {
            let tex = *ctx.texture_exprs.get(texture_id).unwrap_or_else(|| {
                panic!("TextureStore: texture '{}' not in context", texture_id)
            });
        ...
        StatementIR::If { condition, accept, reject } => {
            ...
            // Can't use if_then_else (two closures both capturing &mut scope).
            // Build accept block first, then reject block sequentially.
            bb.if_then(cond, |nested| {
                translate_statements_body(nested, ctx, accept, scope);
            });
            // If there's a reject branch, we need the full If statement.
            // TODO: The above emits If with empty reject. For non-empty reject,
            // we need to build both blocks manually or restructure.
            // For Gate 2, reject branches are uncommon in compute shaders.
            if !reject.is_empty() {
                // Workaround: emit the reject as a separate if(!cond) block
                let false_lit = bb.lit_bool(false);
                let not_cond = bb.eq(cond, false_lit);
                bb.if_then(not_cond, |nested| {
                    translate_statements_body(nested, ctx, reject, scope);
                });
            }
        }
        StatementIR::For { init, condition, update, body } => {
            // WGSL `for` is sugar — naga represents as loop with break_if.
            // Emit init outside loop, then loop { break_if(!cond); body; update; }
            translate_statement_body(bb, ctx, init, scope);
            bb.loop_body(|loop_bb| {
                // break_if(!condition) — break_if breaks when expression is true
                let cond = translate_expr_body(loop_bb, ctx, condition, scope);
                let false_lit = loop_bb.lit_bool(false);
                let not_cond = loop_bb.eq(cond, false_lit);
                loop_bb.break_if(not_cond);
                translate_statements_body(loop_bb, ctx, body, scope);
                translate_statement_body(loop_bb, ctx, update, scope);
            });
        }
        StatementIR::Break => {
            bb.emit_break();
        ...
        StatementIR::AtomicOpField { op, symbol_id, index, value, assign_result_to } => {
            let sym = ctx.symbol_map.get(symbol_id).unwrap_or_else(|| {
            ...
            let offset_lit = bb.lit_u32(sym.word_offset);
            let addr = bb.add(offset_lit, idx);
            let pointer = bb.access(buffer_expr, addr);
            let result = bb.atomic(pointer, map_atomic_function(op), val, *ctx.type_handles.get("u32").expect("u32 missing"));
            if let Some(target_name) = assign_result_to {
                if let Some((target_expr, needs_load)) = scope.resolve(target_name) {
                    if needs_load {
                        bb.store_local(target_expr, result);
                    } else {
                        scope.insert_let(target_name.clone(), result);
                    }
                } else {
                    scope.insert_let(target_name.clone(), result);
                }
        ...
        StatementIR::AtomicOpScalar { op, symbol_id, value, assign_result_to } => {
            let sym = ctx.symbol_map.get(symbol_id).unwrap_or_else(|| {
                panic!("AtomicOpScalar: symbol '{}' not in symbol_map", symbol_id)
            });
            let scalars = match sym.buffer_kind {
                BufferKind::ArenaScalar => {
                    ctx.scalars_expr.expect("AtomicOpScalar requires scalars buffer")
                }
                BufferKind::ArenaScalarAtomic => {
                    ctx.atomic_scalars_expr.expect("AtomicOpScalar requires atomic scalars buffer")
                }
                _ => {
                    resolve_storage_buffer_expr(ctx, sym, symbol_id)
                }
            };
            let val = translate_expr_body(bb, ctx, value, scope);
            let offset = bb.lit_u32(sym.word_offset);
...
fn translate_statements_vertex(
    ...
    fb.with_root(|bb| {
        for stmt in stmts {
            match stmt {
                StatementIR::ReturnVertex { position, varyings: _ } => {
...
fn translate_expr_body(
    ...
    match expr {
        ...
        ExprIR::VarRef { name } => {
            let (expr, needs_load) = scope.resolve(name).unwrap_or_else(|| {
                panic!("VarRef: variable '{}' not found in scope", name)
            });
            if needs_load { bb.load_local(expr) } else { expr }
        }

        ExprIR::LoadGlobal { symbol_id } => {
            let sym = ctx.symbol_map.get(symbol_id).unwrap_or_else(|| {
                panic!("LoadGlobal: symbol '{}' not in symbol_map", symbol_id)
            });
            let globals = ctx.globals_expr.expect("LoadGlobal requires globals buffer");
            let offset = bb.lit_u32(sym.word_offset);
        ...
        ExprIR::LoadScalar { symbol_id } => {
            let sym = ctx.symbol_map.get(symbol_id).unwrap_or_else(|| {
                panic!("LoadScalar: symbol '{}' not in symbol_map", symbol_id)
            });
            let offset = bb.lit_u32(sym.word_offset);
            let raw = match sym.buffer_kind {
                BufferKind::ArenaScalar => {
                    let scalars = ctx.scalars_expr.expect("LoadScalar requires scalars buffer");
                    bb.load_buffer(scalars, offset)
                }
                BufferKind::ArenaScalarAtomic => {
                    let scalars = ctx
                        .atomic_scalars_expr
                        .expect("LoadScalar requires atomic scalars buffer");
                    let pointer = bb.access(scalars, offset);
                    bb.load_local(pointer)
                }
                _ => {
                    panic!(
                        "LoadScalar symbol '{}' points to non-scalar buffer kind {:?}",
                        symbol_id, sym.buffer_kind
                    );
                }
            };
            if sym.wgsl_type == "f32" { bb.bitcast_f32(raw) } else { raw }
        }

        ExprIR::LoadField { symbol_id, index } => {
            let sym = ctx.symbol_map.get(symbol_id).unwrap_or_else(|| {
                panic!("LoadField: symbol '{}' not in symbol_map", symbol_id)
            });
            let buffer_expr = resolve_storage_buffer_expr(ctx, sym, symbol_id);
            let idx = translate_expr_body(bb, ctx, index, scope);
            let offset_lit = bb.lit_u32(sym.word_offset);
            let addr = bb.add(offset_lit, idx);
            let raw = bb.load_buffer(buffer_expr, addr);
            if sym.is_atomic || sym.wgsl_type != "f32" { raw } else { bb.bitcast_f32(raw) }
        }
        ...
        ExprIR::AtomicLoadScalar { symbol_id } => {
            ...
            let scalars = match sym.buffer_kind {
                BufferKind::ArenaScalar => {
                    ctx.scalars_expr.expect("AtomicLoadScalar requires scalars buffer")
                }
                BufferKind::ArenaScalarAtomic => {
                    ctx.atomic_scalars_expr.expect("AtomicLoadScalar requires atomic scalars buffer")
                }
                _ => {
                    resolve_storage_buffer_expr(ctx, sym, symbol_id)
                }
        ...
        ExprIR::CallBuiltin { func, args } => {
            ...
            match func.as_str() {
                ...
                "dpdx" => {
                    if ctx.stage != ShaderTranslationStage::Fragment {
                        panic!("dpdx is only valid in fragment stage");
                    }
                    bb.dpdx(translated[0])
                }
                "dpdy" => {
                    if ctx.stage != ShaderTranslationStage::Fragment {
                        panic!("dpdy is only valid in fragment stage");
                    }
                    bb.dpdy(translated[0])
                }
                "fwidth" => {
                    if ctx.stage != ShaderTranslationStage::Fragment {
                        panic!("fwidth is only valid in fragment stage");
                    }
                    bb.fwidth(translated[0])
                }
                "hash_u32" => {
                    let k0 = bb.lit_u32(747_796_405);
                    let k1 = bb.lit_u32(2_891_336_453);
                    let mul0 = bb.mul(translated[0], k0);
                    let h0 = bb.add(mul0, k1);
                    let shift_bits = bb.lit_u32(16);
                    let h0_shifted = bb.shr(h0, shift_bits);
                    let mixed0 = bb.bit_xor(h0_shifted, h0);
                    let k2 = bb.lit_u32(2_654_435_769);
                    let h1 = bb.mul(mixed0, k2);
                    let shift_bits_2 = bb.lit_u32(16);
                    let h1_shifted = bb.shr(h1, shift_bits_2);
                    bb.bit_xor(h1_shifted, h1)
                }
                "noise_simplex_2d" => {
                    let s0 = bb.lit_f32(12.9898);
                    let s1 = bb.lit_f32(78.233);
                    let seed = bb.compose(
                        *ctx.type_handles.get("vec2<f32>").expect("vec2<f32> missing"),
                        vec![s0, s1],
                    );
                    let dot = bb.dot(translated[0], seed);
                    let s = bb.sin(dot);
                    let scale = bb.lit_f32(43_758.547);
                    let scaled = bb.mul(s, scale);
                    bb.fract(scaled)
                }
                "noise_simplex_3d" => {
                    let s0 = bb.lit_f32(12.9898);
                    let s1 = bb.lit_f32(78.233);
                    let s2 = bb.lit_f32(37.719);
                    let seed = bb.compose(
                        *ctx.type_handles.get("vec3<f32>").expect("vec3<f32> missing"),
                        vec![s0, s1, s2],
                    );
                    let dot = bb.dot(translated[0], seed);
                    let s = bb.sin(dot);
        ...
        ExprIR::TextureSample { texture_id, sampler_id, uv } => {
            let tex = *ctx.texture_exprs.get(texture_id).unwrap_or_else(|| {
        ...
        ExprIR::TextureLoad { texture_id, coords } => {
            let tex = *ctx.texture_exprs.get(texture_id).unwrap_or_else(|| {
                panic!("TextureLoad: texture '{}' not in context", texture_id)
            });
            let coords_expr = translate_expr_body(bb, ctx, coords, scope);
            // Sampled textures require a mip level; storage textures don't.
            // Always pass level 0 — storage textures will ignore it via None.
            let level = bb.lit_i32(0);
            bb.texture_load(tex, coords_expr, Some(level))
        }

        ExprIR::Intrinsic { name } => {
            match name.as_str() {
                "global_invocation_id.x" | "global_invocation_id.y" | "global_invocation_id.z" => {
                    let (gid, _) = scope.resolve("__gid").unwrap_or_else(|| {
                        panic!("Intrinsic '{}' requires compute context", name)
                    });
                ...
                "vertex_index" | "instance_index" => {
                    ...
                    let (expr, _) = scope.resolve(&key).unwrap_or_else(|| {
                        panic!("Intrinsic '{}' not in scope (missing vertex/render context)", name)
                    });
                    expr
                }
        ...
        ExprIR::Cast { target_type, expr: inner } => {
            let e = translate_expr_body(bb, ctx, inner, scope);
