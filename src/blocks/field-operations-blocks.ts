...
registerBlock({
  ...
  lower: ({ ctx, inputsById }) => {
    ...
    if (input.k === 'sig') {
      ...
      const result = ctx.b.fieldMap(input.id, sinFn, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const outType = ctx.outTypes[0];
      const slot = ctx.b.allocSlot();
      return {
        outputsById: {
...
registerBlock({
  ...
  lower: ({ ctx, inputsById }) => {
    ...
    if (input.k === 'sig') {
      ...
      const result = ctx.b.fieldMap(input.id, cosFn, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const outType = ctx.outTypes[0];
      const slot = ctx.b.allocSlot();
      return {
        outputsById: {
...
registerBlock({
  ...
  lower: ({ ctx, inputsById }) => {
    ...
    if (a.k === 'sig' && b.k === 'sig') {
      ...
      const result = ctx.b.fieldZip([a.id, b.id], modFn, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const outType = ctx.outTypes[0];
      const slot = ctx.b.allocSlot();
      return {
        outputsById: {
...
registerBlock({
  ...
  lower: ({ ctx, inputsById }) => {
    ...
    if (angle.k === 'sig' && radius.k === 'sig') {
      ...
      const centerXField = ctx.b.Broadcast(centerXSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const centerYField = ctx.b.Broadcast(centerYSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      ...
      const angleField = angle.k === 'field' ? angle.id : ctx.b.Broadcast((angle as { k: 'sig'; id: SigExprId }).id, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const radiusField = radius.k === 'field' ? radius.id : ctx.b.Broadcast((radius as { k: 'sig'; id: SigExprId }).id, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const centerXField = ctx.b.Broadcast(centerXSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const centerYField = ctx.b.Broadcast(centerYSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
...
registerBlock({
  ...
  lower: ({ ctx, inputsById }) => {
    ...
    if (id01.k === 'sig') {
      ...
      const phaseField = ctx.b.Broadcast(phaseSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const baseField = ctx.b.Broadcast(baseSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const ampField = ctx.b.Broadcast(ampSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const spreadField = ctx.b.Broadcast(spreadSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
...
registerBlock({
  ...
  lower: ({ ctx, inputsById }) => {
    ...
    if (id01.k === 'sig') {
      ...
      const result = ctx.b.fieldZipSig(id01.id, [turnsSig], goldenAngleFn, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
...
registerBlock({
  ...
  lower: ({ ctx, inputsById }) => {
    ...
    if (id01.k === 'sig') {
      ...
      // Field path - broadcast signal inputs to fields
      const phaseField = ctx.b.Broadcast(phaseSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const spinField = ctx.b.Broadcast(spinSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
...
registerBlock({
  ...
  lower: ({ ctx, inputsById }) => {
    ...
    if (id01.k === 'sig' && radius.k === 'sig') {
      ...
      // Mixed path - broadcast signals to fields
      const id01Field = id01.k === 'field' ? id01.id : ctx.b.Broadcast((id01 as { k: 'sig'; id: SigExprId }).id, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const radiusField = radius.k === 'field' ? radius.id : ctx.b.Broadcast((radius as { k: 'sig'; id: SigExprId }).id, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
...
registerBlock({
  ...
  lower: ({ ctx, inputsById }) => {
    ...
    if (pos.k === 'sig' && rand.k === 'sig') {
      ...
      const amountXField = ctx.b.Broadcast(amountXSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const amountYField = ctx.b.Broadcast(amountYSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const amountZField = ctx.b.Broadcast(amountZSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      ...
      // Mixed path - broadcast signals to fields
      const posField = pos.k === 'field' ? pos.id : ctx.b.Broadcast((pos as { k: 'sig'; id: SigExprId }).id, canonicalField(VEC3, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const randField = rand.k === 'field' ? rand.id : ctx.b.Broadcast((rand as { k: 'sig'; id: SigExprId }).id, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const amountXField = ctx.b.Broadcast(amountXSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const amountYField = ctx.b.Broadcast(amountYSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const amountZField = ctx.b.Broadcast(amountZSig, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
...
registerBlock({
  ...
  lower: ({ ctx, inputsById }) => {
    ...
    if (pos.k === 'sig' && z.k === 'sig') {
      ...
      // Mixed path - broadcast signals to fields
      const posField = pos.k === 'field' ? pos.id : ctx.b.Broadcast((pos as { k: 'sig'; id: SigExprId }).id, canonicalField(VEC3, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
      const zField = z.k === 'field' ? z.id : ctx.b.Broadcast((z as { k: 'sig'; id: SigExprId }).id, canonicalField(FLOAT, { kind: 'scalar' }, { instanceId: makeInstanceId('default'), domainType: makeDomainTypeId('default') }));
