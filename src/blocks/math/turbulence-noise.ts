/**
 * TurbulenceNoise1D Block
 *
 * Absolute-value fbm noise in [0,1].
 */
...
const TURBULENCE_NOISE_CARD = cardinalityVar(cardinalityVarId('turbulence_noise_cardinality'), {
...
registerBlock({
  type: 'TurbulenceNoise1D',
  label: 'Turbulence Noise',
  category: 'math',
  description: 'Absolute fbm-style 1D turbulence noise',
  form: 'primitive',
  capability: 'pure',
  loweringPurity: 'pure',
  inputs: {
    x: { label: 'X', type: canonicalType(FLOAT, undefined, { cardinality: TURBULENCE_NOISE_CARD }) },
    seed: {
      label: 'Seed',
      type: canonicalType(FLOAT, undefined, { cardinality: TURBULENCE_NOISE_CARD }),
      defaultValue: 0,
  ...
  outputs: {
    out: { label: 'Out', type: canonicalType(FLOAT, undefined, { cardinality: TURBULENCE_NOISE_CARD }) },
  },
  lower: ({ ctx, inputsById, config }) => {
    const x = inputsById.x;
    const seed = inputsById.seed;
    if (!x || !seed) throw new Error('TurbulenceNoise1D requires x and seed inputs');
    ...
    for (let i = 0; i < octaves; i++) {
      ...
      const octaveSeed = ctx.b.constant(floatConst(i * 97.31), canonicalType(FLOAT));
      ...
      const nAbs = ctx.b.mapAuto(n11, abs, outType);

      const weighted = ctx.b.zipAuto([nAbs, amp], mul, outType);
