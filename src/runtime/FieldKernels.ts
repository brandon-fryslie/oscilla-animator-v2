 ...
 *   polygonVertex, starVertex, lineLayout, gridLayout, circleLayoutUV, lineLayoutUV, gridLayoutUV, fieldGoldenAngle
 *
 * ──────────────────────────────────────────────────────────────────────
 * ARCHITECTURAL RULES
 * ──────────────────────────────────────────────────────────────────────
...
export function applyFieldKernelZipSig(
  ...
  if (fieldOp === 'applyOpacity') {
    ...
    // ────────────────────────────────────────────────────────────────
    // Field input: uv (vec2, stride 2) - UV coordinates from placement basis
    // Signals: [radius: float, phase: float]
    // Output: vec3 (stride 3, z=0.0 explicit)
    // Domain: uv in [0,1]x[0,1], phase in [0,1] for full rotation
    // Coord-space: Produces WORLD-SPACE positions centered at (0.5, 0.5, 0.0)
    // Formula: angle = 2π*(u+phase), pos = center + radius*(cos,sin,0)
    //   Uses u component of UV for angle, ignoring v (for gauge invariance)
    // ════════════════════════════════════════════════════════════════
    if (sigValues.length !== 2) {
    ...
    for (let i = 0; i < N; i++) {
      const u = Math.max(0, Math.min(1, uvArr[i * 2 + 0])); // Clamp u to [0,1]
    ...
    // Signals: [x0: float, y0: float, x1: float, y1: float]
    // Output: vec3 (stride 3, z=0.0 explicit)
    // Domain: uv in [0,1]x[0,1] for interpolation
    // Coord-space: Produces WORLD-SPACE positions in [0,1]
    // Formula: lerp from (x0,y0,0) to (x1,y1,0) using u component
    //   x = (1-u)*x0 + u*x1
    //   y = (1-u)*y0 + u*y1
    //   z = 0.0
    // ════════════════════════════════════════════════════════════════
    ...
    // Signals: [cols: float, rows: float]
    // Output: vec3 (stride 3, z=0.0 explicit)
    // Domain: cols >= 1, rows >= 1, uv in [0,1]x[0,1]
    // Coord-space: Produces WORLD-SPACE positions in [0,1]
    // Formula:
    //   col = floor(u * cols)
    //   row = floor(v * rows)
    ...
    for (let i = 0; i < N; i++) {
      ...
      const col = Math.min(Math.floor(u * cols), cols - 1); // Clamp to valid col
      const row = Math.min(Math.floor(v * rows), rows - 1); // Clamp to valid row
